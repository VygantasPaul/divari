...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	<tr>
		...
		if ( $show_price || $show_price_variations ) :
			...
			<th class="product-price">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'pandastore' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_quantity ) :
			...
			<th class="product-quantity">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'pandastore' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_stock_status ) :
			...
			<th class="product-stock-status">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock status', 'pandastore' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_last_column ) :
			++ $column_count;
			?>
			<th class="product-add-to-cart"><?php esc_html_e( 'Actions', 'pandastore' ); ?></th>
		<?php endif; ?>
		...
		if ( $enable_drag_n_drop ) :
			...
			<th class="product-arrange">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'pandastore' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
	</tr>
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<td class="product-thumbnail">
						<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">
							<?php echo alpha_strip_script_tags( $product->get_image() ); ?>
						</a>
					</td>
					...
					<?php if ( $show_price || $show_price_variations ) : ?>
						<td class="product-price">
							<?php
							if ( $show_price ) {
								echo alpha_strip_script_tags( $item->get_formatted_product_price() );
							}

							if ( $show_price_variations ) {
								echo alpha_strip_script_tags( $item->get_price_variation() );
							}
							?>
						</td>
					<?php endif ?>
					...
					<?php if ( $show_quantity ) : ?>
						<td class="product-quantity">
							...
							<?php else : ?>
								<?php echo alpha_strip_script_tags( $item->get_quantity() ); ?>
							<?php endif; ?>
						</td>
					<?php endif; ?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							<?php echo 'out-of-stock' == $stock_status ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'pandastore' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'pandastore' ) ) . '</span>'; ?>
						</td>
					<?php endif ?>
						...
						<td class="product-add-to-cart">
							<div class="d-lg-flex">
								...
								if ( $show_dateadded && $item->get_date_added() ) :
									echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on: %s', 'pandastore' ), $item->get_date_added_formatted() ) . '</span>';
								endif;
								?>
								...
								if ( ! is_array( $show_info ) || in_array( 'quickview', $show_info ) ) {
									echo '<button class="btn btn-quickview btn-outline btn-dim btn-sm mr-lg-2" data-mfp-src="' . alpha_get_product_featured_image_src( $product ) . '" data-product="' . $product->get_id() . '" title="' . esc_attr__( 'Quick View', 'pandastore' ) . '">' . esc_html__( 'Quick View', 'pandastore' ) . '</button>';
								}
								?>
								...
								<?php if ( $show_add_to_cart && isset( $stock_status ) && $stock_status != 'out-of-stock' ) : ?>
									...
									woocommerce_template_loop_add_to_cart(
										array(
											...
											'class'    => 'btn btn-dim btn-sm mr-lg-2',
										)
									);
									?>
								<?php endif ?>
								...
								<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
									<?php if ( 'select' == $move_to_another_wishlist_type ) : ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php esc_html_e( 'Move', 'pandastore' ); ?></option>
											<?php
									...
									<?php else : ?>
										<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
											<?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'pandastore' ) ); ?>
										</a>
									<?php endif; ?>
								<?php endif; ?>
							</div>
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist p-icon-times-solid" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_attr__( 'Remove this product', 'pandastore' ) ); ?>"></a>
						</td>
					<?php endif; ?>
				</tr>
				<?php
		...
		?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><i class="far fa-heart empty-icon"></i></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'pandastore' ) ); ?></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>">
				<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
					<?php esc_html_e( 'GO TO SHOP', 'pandastore' ); ?>
				</a>
			</td>
		</tr>
		<?php
	...
	if ( ! empty( $page_links ) ) :
		...
		<tr class="pagination-row">
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><?php echo alpha_strip_script_tags( $page_links ); ?></td>
		</tr>
	<?php endif ?>
	</tbody>
