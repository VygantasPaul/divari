...
<tr class="woocommerce-shipping-totals shipping">
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>" colspan="2">
		<?php if ( $available_methods ) : ?>
			<h4><?php esc_html_e( 'Calculate Shipping', 'pandastore' ); ?></h4>
			<ul id="shipping_method" class="woocommerce-shipping-methods">
				<?php foreach ( $available_methods as $method ) : ?>
					<li class="custom-input">
						...
						printf(
							'<div id="shipping_method_%2$s" class="shipping-method">%3$s <label for="shipping_method_%1$s_%2$s">%4$s</label></div>',
							$index,
							esc_attr( sanitize_title( $method->id ) ),
							esc_html__('By courier: ','pandastore'),
