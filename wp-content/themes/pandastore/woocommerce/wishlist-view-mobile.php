				...
				<li id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<div class="item-wrapper">
						<div class="product-thumbnail">
							...
							<?php if ( $show_remove_product ) : ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist p-icon-times-solid" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'pandastore' ) ); ?>"></a>
							<?php endif; ?>
						</div>
					...
					<div class="additional-info-wrapper">
						...
						if ( ! is_array( $show_info ) || in_array( 'quickview', $show_info ) ) {
							echo '<button class="btn btn-quickview btn-outline btn-dim btn-sm mb-2" data-mfp-src="' . alpha_get_product_featured_image_src( $product ) . '" data-product="' . $product->get_id() . '" title="' . esc_attr__( 'Quick View', 'pandastore' ) . '">' . esc_html__( 'Quick View', 'pandastore' ) . '</button>';
						}
						?>
