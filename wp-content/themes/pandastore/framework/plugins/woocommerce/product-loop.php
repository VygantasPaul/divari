...
add_action( 'woocommerce_after_shop_loop_item', 'alpha_product_loop_hide_details', 20 );
...
if ( defined( 'YITH_WCWL' ) ) {
	...
	add_filter( 'yith_wcwl_add_to_wishlist_params', 'alpha_yith_wcwl_add_btn_product_icon_class' );
}
...
if ( ! function_exists( 'alpha_product_loop_vertical_action' ) ) {
	function alpha_product_loop_vertical_action() {
		...
		$exclude_type = apply_filters( 'alpha_product_loop_vertical_action', array( 'product-5', 'product-6', 'product-8', 'list', 'widget' ) );
		if ( ! in_array( $product_type, $exclude_type ) ) {
			...
			'' == alpha_wc_get_loop_prop( 'quickview_pos' ) ) {
				...
				$html .= '<button class="btn-product-icon btn-quickview" data-mfp-src="' . esc_url( alpha_get_product_featured_image_src( $product ) ) . '" data-product="' . $product->get_id() . '" title="' . esc_attr__( 'Quick View', 'pandastore' ) . '">' . esc_html__( 'Quick View', 'pandastore' ) . '</button>';
			}
...
if ( ! function_exists( 'alpha_product_loop_default_wishlist_action' ) ) {
	function alpha_product_loop_default_wishlist_action() {
		$show_info = alpha_wc_get_loop_prop( 'show_info', false );
		if ( defined( 'YITH_WCWL' ) &&
				'with_title' == alpha_wc_get_loop_prop( 'wishlist_pos' ) &&
...
if ( ! function_exists( 'alpha_product_loop_action' ) ) {
	function alpha_product_loop_action( $details = '' ) {
		global $product;
		$product_type = alpha_wc_get_loop_prop( 'product_type' );
		$exclude_type = apply_filters( 'alpha_product_loop_action', array( 'product-5', 'product-6', 'list', 'widget' ) );
		if ( ! in_array( $product_type, $exclude_type ) ) {
...
if ( ! function_exists( 'alpha_get_rating_html' ) ) {
	function alpha_get_rating_html( $html, $rating, $count ) {
		if ( 0 == $rating ) {
			...
			$label = sprintf( esc_html__( 'Rated %s out of 5', 'pandastore' ), $rating );
...
/**
 * Alpha Product Loop Hide Details (for classic type) Functions
 *
 * @since 1.0
 */
if ( ! function_exists( 'alpha_product_loop_hide_details' ) ) {
	function alpha_product_loop_hide_details() {
		if ( alpha_wc_get_loop_prop( 'is_popup' ) && 'list' != alpha_wc_get_loop_prop( 'product_type' ) ) {
			echo '<div class="product-hide-details">';
			do_action( 'alpha_product_loop_hide_details', 'hide-details' );
			echo '</div>';
		}
	}
}
 ...
 * @since 1.0
 */
if ( ! function_exists( 'alpha_yith_wcwl_add_btn_product_icon_class' ) ) {
	function alpha_yith_wcwl_add_btn_product_icon_class( $args ) {
...
if ( ! function_exists( 'alpha_product_compare' ) ) {
	function alpha_product_compare( $extra_class = '' ) {
		...
		if ( Alpha_Product_Compare::get_instance()->is_compared_product( $product_id ) ) {
			...
			$css_class  .= ' added';
			$button_text = apply_filters( 'alpha_woocompare_added_label', esc_html__( 'Added', 'pandastore' ) );
		} else {
			$button_text = apply_filters( 'alpha_woocompare_add_label', esc_html__( 'Compare', 'pandastore' ) );
		}

		printf( '<a href="%s" class="%s" title="%s" data-product_id="%d" data-added-text="%s"></a>', esc_url( $url ), esc_attr( $css_class ), esc_attr( $button_text ), $product_id, esc_html( apply_filters( 'alpha_woocompare_added_label', esc_html__( 'Added', 'pandastore' ) ) ) );
	}
}
...
if ( ! function_exists( 'alpha_get_extra_info_html' ) ) {
	function alpha_get_extra_info_html() {
		...
		if ( ! empty( $extra_info ) ) {
			echo sprintf( '<div class="product-extra-info">(%s)</div>', esc_html( $extra_info ) );
		}
	}
}
...
foreach ( apply_filters(
	...
	if ( $key && $value ) {
		$type = substr( $key, strpos( $key, 'product-' ) === false ? 0 : strpos( $key, 'product-' ) + 8 );
		require_once alpha_framework_path( ALPHA_FRAMEWORK_PLUGINS . "/woocommerce/product-loop/product-loop-{$type}.php" );
	}
}
