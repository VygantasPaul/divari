...
if ( ! function_exists( 'alpha_wc_shop_top_sidebar' ) ) {
	function alpha_wc_shop_top_sidebar() {
		...
		$catalog_orderby_options = apply_filters(
			...
			array(
				'menu_order' => esc_html__( 'Default sorting', 'pandastore' ),
				'popularity' => esc_html__( 'Sort by popularity', 'pandastore' ),
				'rating'     => esc_html__( 'Sort by average rating', 'pandastore' ),
				'date'       => esc_html__( 'Sort by latest', 'pandastore' ),
				'price'      => esc_html__( 'Sort by price: low to high', 'pandastore' ),
				'price-desc' => esc_html__( 'Sort by price: high to low', 'pandastore' ),
			)
		);
		...
		if ( alpha_wc_get_loop_prop( 'is_search' ) ) {
			$catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'pandastore' ) ), $catalog_orderby_options );
...
if ( ! function_exists( 'alpha_clear_cart_action' ) ) {
	...
	function alpha_clear_cart_action() {
		...
		if ( ! empty( $_POST['clear_cart'] ) && wp_verify_nonce( wc_get_var( $_REQUEST['woocommerce-cart-nonce'] ), 'woocommerce-cart' ) ) {
			WC()->cart->empty_cart();
			wc_add_notice( esc_html__( 'Cart is cleared.', 'pandastore' ) );
...
if ( ! function_exists( 'alpha_formatted_address_replacements' ) ) {
	function alpha_formatted_address_replacements( $replacements, $args ) {
		...
		$alpha_customer_address = array(
			__( 'Name', 'pandastore' )     => $args['first_name'] . ' ' . $args['last_name'],
			__( 'Company', 'pandastore' )  => $args['company'],
			__( 'Address', 'pandastore' )  => $args['address_1'] . ' ' . $args['address_2'],
			__( 'City', 'pandastore' )     => isset( $full_state ) ? $args['city'] . ', ' . $full_state : $args['city'],
			__( 'Country', 'pandastore' )  => $full_country,
			__( 'Postcode', 'pandastore' ) => $args['postcode'],
			__( 'Phone', 'pandastore' )    => isset( $args['phone'] ) ? $args['phone'] : '',
		);
		...
		return array(
			...
			'{name}'             => sprintf(
				...
				_x( '%1$s %2$s', 'full name', 'pandastore' ),
				$args['first_name'],
				$args['last_name']
			),
...
if ( ! function_exists( 'alpha_woocommerce_account_menu_items' ) ) {
	...
	function alpha_woocommerce_account_menu_items( $items ) {
		...
		if ( defined( 'YITH_WCWL' ) ) {
			$items['wishlist'] = esc_html__( 'Wishlist', 'pandastore' );
		}

		if ( defined( 'ALPHA_VENDORS' ) ) {
			$items['vendor_dashboard'] = esc_html__( 'Vendor Dashboard', 'pandastore' );
		}
...
if ( ! function_exists( 'alpha_yith_update_mini_wishlist' ) ) {
	...
	function alpha_yith_update_mini_wishlist() {
		...
		if ( defined( 'ALPHA_HEADER_BUILDER' ) ) {
			...
			require ALPHA_HEADER_BUILDER . '/widgets/wishlist/render-wishlist-elementor.php';
		}
