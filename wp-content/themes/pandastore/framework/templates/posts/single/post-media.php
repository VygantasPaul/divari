	...
	if ( is_array( $image_ids ) && count( $image_ids ) ) :
		...
		foreach ( $image_ids as $thumbnail_id ) :
			...
			<figure class="post-media">
				<?php
				$size = apply_filters( 'post_thumbnail_size', 'full', $post->ID );
		...
		<figure class="post-media">
			<a href="<?php the_permalink(); ?>">
				<img src="<?php echo ALPHA_ASSETS . '/images/placeholders/' . get_post_type() . '-placeholder.jpg'; ?>" alt="<?php esc_attr_e( 'Post placeholder image', 'pandastore' ); ?>">
			</a>
		</figure>
		<?php
	endif;
