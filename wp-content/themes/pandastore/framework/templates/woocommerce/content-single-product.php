...
} elseif ( apply_filters( 'alpha_is_single_product_widget', false ) ) {
	$view_type = 'widget';
	$classes[] = 'product-widget';
	if ( ( ! alpha_doing_ajax() || alpha_is_elementor_preview() ) && 'gallery' != $single_product_layout && 'sticky-both' != $single_product_layout ) {
		$classes[] = 'row';
	}
} else {
	$view_type = '';
...
<<?php echo boolval( $is_li_tag ) ? 'li' : 'div'; ?> id="product-<?php the_ID(); ?>" <?php wc_product_class( $classes, $product ); ?>>
	...
	if ( ! apply_filters( 'alpha_run_single_product_builder', false ) ) {
		...
		<div class="<?php echo esc_attr( apply_filters( 'alpha_single_product_summary_class', 'summary entry-summary' ) ); ?>">
			 ...
			 * @hooked alpha_print_wishlist_button - 52
		...
		if ( ! $view_type ) {
			 ...
			 * @hooked alpha_dokan_get_more_products_from_seller - 13
