...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo ! $show_cb ? '' : 'with-checkbox'; ?>" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<li id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<div class="item-wrapper">
						...
						<div class="item-details">
							<div class="product-name">
								<h3><?php echo alpha_strip_script_tags( apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ); ?></h3>
							...
							<?php if ( $show_variation || $show_dateadded || $show_price ) : ?>
								<table class="item-details-table">
									...
									<?php if ( $show_dateadded && $item->get_date_added() ) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Added on:', 'pandastore' ); ?>
											</td>
											<td class="value">
												<?php echo alpha_strip_script_tags( $item->get_date_added_formatted() ); ?>
											</td>
										</tr>
									<?php endif; ?>
									...
									<?php if ( $show_price || $show_price_variations ) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Price:', 'pandastore' ); ?>
											</td>
											<td class="value">
												<?php
												if ( $show_price ) {
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_quantity || $show_stock_status ) : ?>
							<table class="additional-info">
								<?php if ( $show_quantity ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Quantity:', 'pandastore' ); ?>
										</td>
										<td class="value">
								...
								<?php if ( $show_stock_status ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Stock:', 'pandastore' ); ?>
										</td>
										<td class="value">
											<?php echo 'out-of-stock' == $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'pandastore' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'pandastore' ) . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
						...
						if ( ! is_array( $show_info ) || in_array( 'quickview', $show_info ) ) {
							echo '<button class="btn btn-quickview btn-outline btn-default btn-rounded btn-sm mt-2" data-mfp-src="' . esc_url( alpha_get_product_featured_image_src( $product ) ) . '" data-product="' . $product->get_id() . '" title="' . esc_attr__( 'Quick View', 'pandastore' ) . '">' . esc_html__( 'Quick View', 'pandastore' ) . '</button>';
						}
						?>
		...
		<p class="wishlist-empty">
			<?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'pandastore' ) ); ?>
		</p>
		<?php
	endif;
	...
	if ( ! empty( $page_links ) ) :
		...
		<p class="pagination-row">
			<?php echo alpha_strip_script_tags( $page_links ); ?>
		</p>
	<?php endif ?>
