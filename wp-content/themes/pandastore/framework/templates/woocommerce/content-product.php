...
$show_in_box       = alpha_wc_get_loop_prop( 'show_in_box' );
$show_media_shadow = alpha_wc_get_loop_prop( 'show_media_shadow' );
$show_hover_shadow = alpha_wc_get_loop_prop( 'show_hover_shadow' );
	...
	if ( isset( $GLOBALS['alpha_current_product_id'] ) ) {
		...
		if ( isset( $repeaters['ids'][ $current_id + 1 ] ) ) {
			...
			if ( $current_id + 1 != $sp_insert && $current_id + 1 != $banner_insert ) {
				...
				$product_type                              = empty( $repeaters['product_type'][ $current_id + 1 ] ) ? $global_product_type : $repeaters['product_type'][ $current_id + 1 ];
...
if ( 'product-5' == $product_type || 'product-6' == $product_type ) {
	...
	if ( 'product-2' == $product_type || 'product-3' == $product_type ) {
		...
		if ( alpha_get_option( 'catalog_mode' ) && ! alpha_get_option( 'catalog_cart' ) && alpha_get_option( 'catalog_price' ) ) {
...
<li class="<?php echo esc_attr( apply_filters( 'alpha_product_wrap_class', $wrap_class ) ); ?>"<?php echo esc_attr( $wrapper_attr ); ?>>
	...
	<div <?php wc_product_class( apply_filters( 'alpha_product_class', $product_classes, $product_type, $show_hover_shadow, $addtocart_pos ), $product ); ?>>
		<?php
		/**
		 ...
		 * @hooked alpha_product_loop_attributes - 20
