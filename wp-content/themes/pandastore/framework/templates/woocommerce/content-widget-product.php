...
$show_info = apply_filters( 'alpha_get_widget_products_show_info', array( 'price', 'rating' ) );
	...
	<div class="product-details">
		...
		<div class="woocommerce-product-rating">
			...
			<?php else : ?>
				<?php
				if ( ! empty( $show_rating ) && in_array( 'rating', $show_info ) ) {
					echo alpha_escaped( $product->get_rating_html() ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
				}
				?>
			<?php endif ?>
		</div>
		...
		<span class="price">
			<?php echo alpha_escaped( $product->get_price_html() );  // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?>
		</span>
		<?php endif; ?>
	</div>
