...
if ( ! apply_filters( 'alpha_is_vendor_store', false ) && alpha_is_shop() && ! alpha_wc_get_loop_prop( 'widget' ) ) {
...
if ( $loadmore_type ) {
	...
	if ( $total_pages > 1 ) {
		if ( 'page' === $loadmore_type ) {
			if ( alpha_wc_get_loop_prop( 'widget', false ) && empty( $alpha_layout['is_shop_builder_rendering'] ) ) {
