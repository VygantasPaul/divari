...
if ( ! is_array( $show_info ) || in_array( 'label', $show_info ) ) {
	...
	if ( is_array( $show_labels ) && count( $show_labels ) ) {
		...
		if ( $product->is_featured() && in_array( 'hot', $show_labels ) ) {
			$html .= '<label class="product-label label-featured">' . esc_html__( 'Hot', 'pandastore' ) . '</label>';
		}

		// New label
		if ( in_array( 'new', $show_labels ) && strtotime( $product->get_date_created() ) > strtotime( '-' . (int) alpha_get_option( 'new_product_period' ) . ' day' ) ) {
			$html .= '<label class="product-label label-new">' . esc_html__( 'New', 'pandastore' ) . '</label>';
		}
		...
		if ( $product->is_on_sale() && in_array( 'sale', $show_labels ) ) {
			...
			if ( $percentage ) {
				...
				$html .= '<label class="product-label label-sale">' . sprintf( esc_html__( '%d%% off', 'pandastore' ), $percentage ) . '</label>';
			}
		}

		// Out of stock label
		if ( in_array( 'stock', $show_labels ) && 'outofstock' == $product->get_stock_status() ) {
			$html .= '<label class="product-label label-stock">' . esc_html__( 'Out of Stock', 'pandastore' ) . '</label>';
		}
	}
}

// Show custom labels
if ( ! is_array( $show_info ) || in_array( 'custom_label', $show_info ) ) {

		$custom_labels = json_decode( get_post_meta( get_the_ID(), 'alpha_custom_labels', true ), true );
