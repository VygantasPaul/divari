...
<?php
	...
	echo '<div class="cart-header">';
		...
		echo '<h4 class="cart-title">' . esc_html__( 'Shopping Cart', 'pandastore' ) . '</h4>';
		echo '<a class="btn btn-dark btn-link btn-icon-right btn-close" href="#">' . esc_html__( 'Close', 'pandastore' ) . '<i class="' . ALPHA_ICON_PREFIX . '-icon-arrow-long-right"></i><span class="sr-only">' . esc_html__( 'Cart', 'pandastore' ) . '</span></a>';
	echo '</div>';
?>
...
<?php if ( ! WC()->cart->is_empty() ) : ?>
				...
				<li class="woocommerce-mini-cart-item mini-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					<div class="cart-item-meta mini-item-meta">
						...
						if ( ! alpha_get_option( 'cart_show_qty' ) ) {
							...
							$product_quantity = sprintf( '<input type="number" name="%1$s" class="qty" value="%2$d" min="%3$d" max="%4$d" />', esc_attr( $cart_item_key ), $cart_item['quantity'], 1, $_product->get_max_purchase_quantity() > 0 ? $_product->get_max_purchase_quantity() : 1000000 );
						}
						echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity">' . sprintf( '%s <span class="times">&times</span> %s', $product_quantity, $product_price ) . '</span>', $cart_item, esc_attr( $cart_item_key ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						?>
					</div>
					<?php
					if ( empty( $product_permalink ) ) :
						...
						echo '<a href="' . esc_url( $product_permalink ) . '">' . alpha_strip_script_tags( $thumbnail ) . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					endif;
					?>
					<?php
	...
	<p class="woocommerce-mini-cart__empty-message empty-msg"><?php esc_html_e( 'No products in the cart.', 'pandastore' ); ?></p>
