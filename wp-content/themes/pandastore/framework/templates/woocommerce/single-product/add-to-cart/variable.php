...
<form class="variations_form cart" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo esc_attr( $variations_attr ); // WPCS: XSS ok. ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php echo esc_html( apply_filters( 'woocommerce_out_of_stock_message', esc_html__( 'This product is currently out of stock and unavailable.', 'pandastore' ) ) ); ?></p>
	<?php else : ?>
		<div class="variations">
			<?php foreach ( $attributes as $attribute_name => $options ) : ?>
				...
				if ( $attribute_id && ( alpha_get_option( 'advanced_swatch' ) || class_exists( 'Alpha_Plugin_Options' ) && Alpha_Plugin_Options::get_option( 'alpha_advanced_swatch', 'yes' ) ) ) {
				...
				<?php
				echo end( $attribute_keys ) == $attribute_name ? alpha_strip_script_tags( apply_filters( 'woocommerce_reset_variations_link', '<div class="reset-variation-cont"><a class="reset_variations d-none" href="#">' . esc_html__( 'Clean All', 'pandastore' ) . '</a></div>' ) ) : '';
				?>
			<?php endforeach; ?>
		</div>
