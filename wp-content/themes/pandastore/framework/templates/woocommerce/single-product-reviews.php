		...
		<div class="row">
			<div class="col-md-4 mb-4">
				...
				<h4 class="avg-rating-container">
					...
					<span class="avg-rating">
						...
						<span class="star-rating">
							<span style="width: <?php echo alpha_escaped( $display_rate ) . '%'; ?>;"><?php esc_html_e( 'Rated', 'pandastore' ); ?></span>
						</span>
						<span class="ratings-review"><?php echo sprintf( esc_html__( '(%s Reviews)', 'pandastore' ), $count ); //phpcs: ignore ?></span> 
					</span>
				</h4>
					...
					for ( $i = 5; $i > 0; $i-- ) {
						...
						<div class="ratings-item">
							<div class="star-rating">
								<span style="width: <?php echo absint( $i ) * 20 . '%'; ?>"><?php esc_html_e( 'Rated', 'pandastore' ); ?></span>
							</div>
				...
				<?php if ( $review_offcanvas && ( 'no' === get_option( 'woocommerce_review_rating_verification_required' ) || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) ) : ?>
					<a class="btn btn-dark submit-review-toggle" href="#"><?php echo esc_html__( 'Submit Review', 'pandastore' ); ?></a>
				<?php endif; ?>
			...
			<div class="col-md-8 mb-4">
				<?php if ( $review_offcanvas ) : ?>
					<div class="select-wrapper pt-4 pt-md-0 pb-4 <?php echo 'section' == apply_filters( 'alpha_single_product_data_tab_type', 'tab' ) ? 'd-md-none' : ''; ?>">
						<span class="select-box">
							<?php echo alpha_strip_script_tags( apply_filters( 'alpha_single_product_review_filter_select_html', '' ) ); ?>
						</span>
					</div>
				...
				<div id="review_form_wrapper" <?php echo ( ! empty( $review_offcanvas ) ? 'class="offcanvas"' : '' ); ?>>
					...
					<div id="review_form" <?php echo ( ! empty( $review_offcanvas ) ? 'class="offcanvas-content"' : '' ); ?>>
						...
						$comment_form = array(
							...
							'title_reply'         => have_comments() ? esc_html__( 'Submit Your Review', 'pandastore' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'pandastore' ), get_the_title() ),
							/* translators: %s is product title */
						...
						if ( wc_review_ratings_enabled() ) {
							$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating Of This Product', 'pandastore' ) . ( wc_review_ratings_required() ? '&nbsp;<span class="required">:</span>' : '' ) . '</label><select name="rating" id="rating" required>
						...
						$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'pandastore' ) . '&nbsp;<span class="required"></span></label><textarea id="comment" name="comment" cols="45" rows="8" required></textarea></p>';
			...
			<?php else : ?>
				<p class="woocommerce-verification-required"><?php esc_html_e( 'Only logged in customers who have purchased this product may leave a review.', 'pandastore' ); ?></p>
			<?php endif; ?>

			<?php if ( $review_offcanvas ) : ?>
				<?php if ( have_comments() ) : ?>
						<ul id="commentlist" class="commentlist" data-empty="<?php esc_attr_e( 'No such review exists.', 'pandastore' ); ?>">
						...
						?>
				<?php else : ?>
					<p class="woocommerce-noreviews"><?php esc_html_e( 'There are no reviews yet.', 'pandastore' ); ?></p>
				<?php endif; ?>
			<?php endif; ?>
		...
		<?php if ( have_comments() && ! $review_offcanvas ) : ?>
				...
				<ul id="alpha_comment_tabs" class="alpha-comment-tabs nav nav-tabs tab-nav-solid" role="tablist">
					<li class="nav-item" role="tab" aria-controls="commentlist">
						<span data-href="#commentlist" class="nav-link active" data-order="default"><?php esc_html_e( 'Show all', 'pandastore' ); ?></span>
					</li>
