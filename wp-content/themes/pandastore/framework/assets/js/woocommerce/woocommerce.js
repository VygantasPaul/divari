...
( function ( $ ) {
    theme.woocommerce = {
        init: function () {
            ...
            theme.call( theme.woocommerce.initProducts.bind( theme.woocommerce ), 500 );
        },
        /**
         * Initialize products
         * - rating tooltip
                ...
                .on( 'click', '.yith-wcwl-wishlistexistsbrowse a, .yith-wcwl-wishlistaddedbrowse a', function ( e ) {
                    var $link = $( e.currentTarget ),
                        ...
                        data = {
                            ...
                            from: 'theme',
                            is_quickview: $link.closest( '.product-single.product-quickview' ).length ? true : false,
                        };
        ...
        ProductGallery.prototype.initZoom = function () {
            if ( alpha_vars.single_product.zoom_enabled ) {
                ...
                if ( !this.$wc_gallery.closest( '.product-quickview' ).length && !this.$wc_gallery.closest( '.product-widget' ).length && !this.$wc_gallery.hasClass( 'woocommerce-product-gallery--without-images' ) ) {
                    var buttons = '<button class="product-gallery-btn product-image-full ' + alpha_vars.theme_icon_prefix + '-icon-zoom" aria-label="Product Gallery"></button>' + ( this.$wc_gallery.data( 'buttons' ) || '' );
                    ...
                    if ( this.$slider.length && this.$slider.hasClass( 'product-single-carousel' ) ) {
                        ...
                        this.$slider.siblings( '.product-image-full' ).length || this.$slider.after( buttons );
                    } else {
                        // else other types
                        this.$wc_gallery.find( '.woocommerce-product-gallery__image > a' ).each( function () {
                            $( this ).siblings( '.product-image-full' ).length || $( this ).after( buttons );
                        } );
                    }
                }
