			...
			var alpha_fn_remove_demo = function ( options, all_checked ) {
				...
				if ( option !== undefined ) {
					...
					$.ajax( {
						...
						failure: function () {
							$( '.alpha-remove-demo .button' ).prop( 'disabled', false );
							$( '.alpha-remove-demo .remove-status' ).html( '<h5>' + alpha_setup_wizard_params.texts.try_again + '</h5>' );
						}
					} );
				} else {
					$( '.alpha-remove-demo .remove-status' ).html( '<h5 class="success">' + alpha_setup_wizard_params.texts.removed + '</h5>' );
					$( '.alpha-remove-demo .button' ).prop( 'disabled', false );
				}
			};
