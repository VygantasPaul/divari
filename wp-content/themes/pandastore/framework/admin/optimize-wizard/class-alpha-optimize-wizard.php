	...
	*/
	class Alpha_Optimize_Wizard extends Alpha_Base {
		...
		public function init_actions() {
			...
			add_action( 'wp_ajax_alpha_optimize_wizard_resources_load', array( $this, 'ajax_load_resources' ) );
		...
		public function add_admin_menu() {
			add_submenu_page( 'alpha', esc_html__( 'Optimize Wizard', 'pandastore' ), esc_html__( 'Optimize Wizard', 'pandastore' ), 'manage_options', $this->page_slug, array( $this, 'view_optimize_wizard' ), 3 ); // phpcs:ignore WPThemeReview.PluginTerritory.NoAddAdminPages
		}
		...
		public function enqueue() {
			...
			wp_localize_script(
				...
				array(
					...
					'texts'            => array(
						'loading_failed' => esc_html__( 'Loading Failed', 'pandastore' ),
		...
		public function view_optimize_wizard() {
			...
			$title        = array(
				'title' => esc_html__( 'Optimize Wizard', 'pandastore' ),
				'desc'  => ALPHA_DISPLAY_NAME . esc_html__( ' optimize wizard will help you configure proper website with optimum resources and peak efficiency.', 'pandastore' ),
			);
		...
		private function get_all_shortcodes() {
			...
			if ( class_exists( 'WPBMap' ) ) {
				...
				if ( ! empty( $all_wpb_shortcodes ) ) {
					foreach ( $all_wpb_shortcodes as $key => $value ) {
						if ( 0 === strpos( $key, 'wpb_alpha' ) || 'vc_section' == $key || 'vc_row' == $key || 'vc_row_inner' == $key || 'vc_column' == $key || 'vc_column_inner' == $key ) {
							continue;
						}
						$shortcodes[] = $key;
					}
			...
			return apply_filters( 'alpha_all_shortcodes', $shortcodes );
		}
		...
		private function get_used_shortcodes( $shortcodes = array(), $return_ids = false, $attrs = array() ) {
			...
			return apply_filters( 'alpha_wpb_get_used_shortcodes', $used );
		}
		...
		public function ajax_deactivate_plugins() {
			...
			if ( ! current_user_can( 'deactivate_plugin', $plugin ) ) {
				wp_die( esc_html__( 'Sorry, you are not allowed to deactivate this plugin.', 'pandastore' ) );
			}
		...
		public function init_wizard_steps() {
			$this->steps = array(
				'resources'   => array(
					'name'    => esc_html__( 'Resources', 'pandastore' ),
					'view'    => array( $this, 'view_resources' ),
					'handler' => '',
				),
				'lazyload'    => array(
					'name'    => esc_html__( 'Lazyload', 'pandastore' ),
					'view'    => array( $this, 'view_lazyload' ),
					'handler' => array( $this, 'view_lazyload_save' ),
				),
				'performance' => array(
					'name'    => esc_html__( 'Peformance', 'pandastore' ),
			...
			if ( class_exists( 'TGM_Plugin_Activation' ) && isset( $GLOBALS['tgmpa'] ) ) {
				$this->steps['plugins'] = array(
					'name'    => esc_html__( 'Plugins', 'pandastore' ),
		...
		public function ajax_load_resources() {
			...
			include alpha_framework_path( ALPHA_FRAMEWORK_ADMIN . '/customizer/dynamic/dynamic_conditions.php' );
			foreach ( $checked_elements as $element => $used ) {
				if ( ! ( isset( $alpha_used_elements[ $element ] ) && $alpha_used_elements[ $element ] ) ) {
					$alpha_used_elements[ $element ] = $used;
				}
			}
			...
			include alpha_framework_path( ALPHA_OPTIMIZE_WIZARD . '/views/resources_load.php' );
			die;
		}
		...
		public function ajax_optimize_resources() {
			...
			if ( alpha_get_feature( 'fs_pb_wpb' ) && defined( 'WPB_VC_VERSION' ) ) {
				...
				alpha_wpb_shortcode_compile_css( $wpb_shortcodes_to_remove );
			}
			// @end feature: fs_pb_wpb
