	...
	class Alpha_WP_Import extends WP_Importer {
		...
		function import( $file, $process = 'import_start', $index = 0 ) {
			...
			switch ( $process ) {
				case 'import_start':
					...
					$response['message'] = esc_html__( 'Imported categories', 'pandastore' );
					$response['process'] = 'process_tags';
					break;
				case 'process_tags':
					$this->process_tags();
					$response['message'] = esc_html__( 'Imported tags', 'pandastore' );
					$response['process'] = 'process_terms';
					break;
				case 'process_terms':
					$this->process_terms();
					$response['message'] = esc_html__( 'Imported terms', 'pandastore' );
					$response['process'] = 'process_posts';
					break;
				case 'process_posts':
					if ( $index >= count( $this->posts ) ) {
						$this->backfill_parents();
						$response['message'] = esc_html__( 'Imported posts', 'pandastore' );
						$response['process'] = 'process_menus';
						$response['index']   = 0;
					} else {
						$response['count'] = count( $this->posts );
						$this->process_posts( $index );
						$response['message'] = esc_html__( 'Importing posts', 'pandastore' );
						$response['process'] = 'process_posts';
				...
				case 'process_menus':
					if ( $index >= count( $this->missing_menu_items ) ) {
						...
						$response['message'] = esc_html__( 'Importing menus', 'pandastore' );
						$response['process'] = 'process_menus_orphans';
						$response['index']   = ++$index;
					} else {
						$response['count'] = count( $this->missing_menu_items );
						$this->process_menus( $index );
						$response['message'] = esc_html__( 'Processing menus', 'pandastore' );
						$response['process'] = 'process_menus';
						$response['index']   = ++$index;
					}
					break;
				...
				case 'remap_featured_images':
					...
					$response['message'] = esc_html__( 'Successfully imported dummy content!', 'pandastore' );
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'pandastore' ), '<code>' . esc_html( $file['file'] ) . '</code>' );
				echo '</p>';
				return false;
			}
			...
			try {
				...
				foreach ( $this->posts as $post ) {
					if ( 'ptu_tax' == $post['post_type'] ) {
						...
						// labels array
						$labels = array(
							'name'                       => _x( sprintf( '%s', $label ), 'taxonomy general name', 'pandastore' ),
							'singular_name'              => _x( sprintf( '%s', $singular_name ), 'taxonomy singular name', 'pandastore' ),
							'all_items'                  => $all_items,
							'edit_item'                  => $edit_item,
							'view_item'                  => $view_item,
