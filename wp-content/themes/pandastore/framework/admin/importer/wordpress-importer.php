...
require alpha_framework_path( ALPHA_FRAMEWORK_ADMIN . '/importer/parsers.php' );
...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		...
		function import_start( $file ) {
			if ( ! is_file( $file ) ) {
				echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'pandastore' ) . '</strong><br />';
				esc_html_e( 'The file does not exist, please try again.', 'pandastore' ) . '</p>';
				$this->footer();
				die();
			}
		...
		function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'pandastore' ) . ' <a href="' . esc_url( admin_url() ) . '">' . esc_html__( 'Have fun!', 'pandastore' ) . '</a>' . '</p>';
			echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'pandastore' ) . '</p>';
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				printf( alpha_strip_script_tags( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'pandastore' ) ), esc_html( $file['file'] ) );
				echo '</p>';
				return false;
			}
			...
			if ( $this->version > $this->max_wxr_version ) {
				...
				printf( esc_html__( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'pandastore' ), esc_html( $import_data['version'] ) );
				echo '</strong></p></div>';
			}
			...
			if ( ! empty( $import_data['authors'] ) ) {
				...
				foreach ( $import_data['posts'] as $post ) {
					...
					if ( empty( $login ) ) {
						...
						printf( esc_html__( 'Failed to import author %s. Their posts will be attributed to the current user.', 'pandastore' ), esc_html( $post['post_author'] ) );
						echo '<br />';
						continue;
					}
...
<form action="<?php echo esc_url( admin_url( 'admin.php?import=wordpress&amp;step=2' ) ); ?>" method="post">
	...
	<h3><?php esc_html_e( 'Assign Authors', 'pandastore' ); ?></h3>
				<?php /* translators: opening and closing code tags */ ?>
	<p><?php printf( esc_html__( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as %1$sadmin%2$ss entries.', 'pandastore' ), '<code>', '</code>' ); ?></p>
				<?php if ( $this->allow_create_users() ) : ?>
					<?php /* translators: %s: default role name */ ?>
	<p><?php printf( esc_html__( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', 'pandastore' ), esc_html( get_option( 'default_role' ) ) ); ?></p>
<?php endif; ?>
	<ol id="authors">
...
<?php endif; ?>
	...
	<h3><?php esc_html_e( 'Import Attachments', 'pandastore' ); ?></h3>
	<p>
		<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
		<label for="import-attachments"><?php esc_html_e( 'Download and import file attachments', 'pandastore' ); ?></label>
	</p>
<?php endif; ?>

	<p class="submit"><input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'pandastore' ); ?>" /></p>
</form>
			<?php
		}
		...
		function author_select( $n, $author ) {
			...
			esc_html_e( 'Import author:', 'pandastore' );
			...
			if ( $create_users ) {
				if ( '1.0' != $this->version ) {
					esc_html_e( 'or create new user with login name:', 'pandastore' );
					$value = '';
				} else {
					esc_html_e( 'as a new user:', 'pandastore' );
			...
			if ( ! $create_users && '1.0' == $this->version ) {
				esc_html_e( 'assign posts to an existing user:', 'pandastore' );
			} else {
				esc_html_e( 'or assign posts to an existing user:', 'pandastore' );
			}
			wp_dropdown_users(
				array(
					'name'            => "user_map[$n]",
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][ $i ] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						printf( esc_html__( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'pandastore' ), esc_html( $this->authors[ $old_login ]['author_display_name'] ) );
		...
		function process_categories() {
			...
			foreach ( $this->categories as $cat ) {
				...
				if ( ! is_wp_error( $id ) && $id > 0 ) {
					...
					printf( esc_html__( 'Failed to import category %s', 'pandastore' ), esc_html( $cat['category_nicename'] ) );
					if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
		...
		function process_tags() {
			...
			foreach ( $this->tags as $tag ) {
				...
				if ( ! is_wp_error( $id ) ) {
					...
					printf( esc_html__( 'Failed to import post tag %s', 'pandastore' ), esc_html( $tag['tag_name'] ) );
					if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
						echo ': ' . $id->get_error_message();
					}
					echo '<br />';
		...
		function process_terms() {
			...
			foreach ( $this->posts as $post ) {
				if ( 'ptu_tax' == $post['post_type'] ) {
					...
					$search_items               = array_key_exists( '_ptu_labels_search_items', $meta ) ? $meta['_ptu_labels_search_items'][0] : sprintf( _x( 'Search %s', 'taxonomy label', 'pandastore' ), $label );
					$all_items                  = array_key_exists( '_ptu_labels_all_items', $meta ) ? $meta['_ptu_labels_all_items'][0] : sprintf( _x( 'All %s', 'taxonomy label', 'pandastore' ), $label );
					$parent_item                = array_key_exists( '_ptu_labels_parent_item', $meta ) ? $meta['_ptu_labels_parent_item'][0] : sprintf( _x( 'Parent %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$parent_item_colon          = array_key_exists( '_ptu_labels_parent_item_colon', $meta ) ? $meta['_ptu_labels_parent_item_colon'][0] : sprintf( _x( 'Parent %s:', 'taxonomy label', 'pandastore' ), $singular_name );
					$view_item                  = array_key_exists( '_ptu_labels_parent_view_item', $meta ) ? $meta['_ptu_labels_parent_view_item'][0] : sprintf( _x( 'View %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$edit_item                  = array_key_exists( '_ptu_labels_parent_edit_item', $meta ) ? $meta['_ptu_labels_parent_edit_item'][0] : sprintf( _x( 'Edit %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$update_item                = array_key_exists( '_ptu_labels_update_item', $meta ) ? $meta['_ptu_labels_update_item'][0] : sprintf( _x( 'Update %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$add_new_item               = array_key_exists( '_ptu_labels_add_new_item', $meta ) ? $meta['_ptu_labels_add_new_item'][0] : sprintf( _x( 'Add New %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$add_new_item_name          = array_key_exists( '_ptu_labels_add_new_item_name', $meta ) ? $meta['_ptu_labels_add_new_item_name'][0] : sprintf( _x( 'New %s', 'taxonomy label', 'pandastore' ), $singular_name );
					$popular_items              = array_key_exists( '_ptu_labels_popular_items', $meta ) ? $meta['_ptu_labels_popular_items'][0] : sprintf( _x( 'Popular %s', 'taxonomy label', 'pandastore' ), $label );
					$separate_items_with_commas = array_key_exists( '_ptu_labels_separate_items_with_commas', $meta ) ? $meta['_ptu_labels_separate_items_with_commas'][0] : sprintf( _x( 'Separate %s with commas', 'taxonomy label', 'pandastore' ), $label );
					$add_or_remove_items        = array_key_exists( '_ptu_labels_add_or_remove_items', $meta ) ? $meta['_ptu_labels_add_or_remove_items'][0] : sprintf( _x( 'Add or remove %s', 'taxonomy label', 'pandastore' ), $label );
					$choose_from_most_used      = array_key_exists( '_ptu_labels_choose_from_most_used', $meta ) ? $meta['_ptu_labels_choose_from_most_used'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'pandastore' ), $label );
					$choose_not_found           = array_key_exists( '_ptu_labels_choose_from_most_used', $meta ) ? $meta['_ptu_labels_choose_from_most_used'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'pandastore' ), $label );
					$not_found                  = array_key_exists( '_ptu_labels_not_found', $meta ) ? $meta['_ptu_labels_not_found'][0] : sprintf( _x( 'Choose from the most used %s', 'taxonomy label', 'pandastore' ), $label );
					$back_to_items              = array_key_exists( '_ptu_labels_back_to_items', $meta ) ? $meta['_ptu_labels_back_to_items'][0] : sprintf( _x( 'Back to %s', 'taxonomy label', 'pandastore' ), $label );

					// labels array
					$labels = array(
						'name'                       => _x( $label, 'taxonomy general name', 'pandastore' ),
						'singular_name'              => _x( $singular_name, 'taxonomy singular name', 'pandastore' ),
						'all_items'                  => $all_items,
						'edit_item'                  => $edit_item,
						'view_item'                  => $view_item,
			...
			foreach ( $this->terms as $term ) {
				...
				if ( ! is_wp_error( $id ) ) {
					...
					printf( esc_html__( 'Failed to import %1$s %2$s', 'pandastore' ), esc_html( $term['term_taxonomy'] ), esc_html( $term['term_name'] ) );
					if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
						echo ': ' . $id->get_error_message();
					}
					echo '<br />';
			...
			foreach ( $this->posts as $post ) {
				...
				if ( ! post_type_exists( $post['post_type'] ) && ! in_array( $post['post_type'], $whitelist ) ) {
					printf(
						...
						esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'pandastore' ),
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					printf( esc_html__( '%1$s &#8220;%2$s&#8221; already exists.', 'pandastore' ), $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
					echo '<br />';
					...
					if ( in_array( $post['post_type'], Alpha_Setup_Wizard::get_instance()->demo_import_post_types ) ) {
						...
						if ( is_wp_error( $post_id ) ) {
							printf(
								esc_html__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'pandastore' ),
						...
						$meta_fields = array( '_wpb_post_custom_css', '_wpb_shortcodes_custom_css', '_elementor_data', '_elementor_css', '_elementor_edit_mode', '_elementor_page_settings', '_elementor_version', 'page_js', 'page_css' );
					...
					foreach ( $post['terms'] as $term ) {
						...
						if ( ! $term_id ) {
							...
							if ( ! is_wp_error( $t ) ) {
								...
								printf( esc_html__( 'Failed to import %1$s %2$s', 'pandastore' ), esc_html( $taxonomy ), esc_html( $term['name'] ) );
								if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				esc_html_e( 'Menu item skipped due to missing menu slug', 'pandastore' );
				echo '<br />';
				return;
			}
			...
			if ( ! $menu_id ) {
				printf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'pandastore' ), esc_html( $menu_slug ) );
				echo '<br />';
				return;
			} else {
		...
		function process_attachment( $post, $url ) {
			if ( ! $this->fetch_attachments ) {
				return new WP_Error(
					...
					esc_html__( 'Fetching attachments is not enabled', 'pandastore' )
				);
			}
			...
			if ( $info = wp_check_filetype( $upload['file'] ) ) {
				...
				return new WP_Error( 'attachment_processing_error', esc_html__( 'Invalid file type', 'pandastore' ) );
			}
			...
			do_action( 'alpha_importer_insert_attachment', $post_id, isset( $post['import_id'] ) ? (int) $post['import_id'] : false );
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( is_wp_error( $http_response ) ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'pandastore' ) );
			}
			...
			if ( '200' != $response['code'] ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote server returned error response %1$d %2$s', 'pandastore' ), esc_html( $response['code'] ), get_status_header_desc( $response['code'] ) ) );
			}
			...
			if ( ( ! empty( $max_size ) && $filesize > $max_size ) || ( ! empty( $max_size ) && isset( $headers['content-length'] ) && $headers['content-length'] > $max_size ) ) {
				@unlink( $upload['file'] );
				return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'pandastore' ), size_format( $max_size ) ) );
			}
		...
		function header() {
			echo '<div class="wrap">';
			echo '<h2>' . esc_html__( 'Import WordPress', 'pandastore' ) . '</h2>';
			...
			if ( isset( $updates[ $basename ] ) ) {
				...
				printf( esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'pandastore' ), $update->update->new_version );
				echo '</strong></p></div>';
			}
		}
		...
		function greet() {
			...
			echo '<p>' . esc_html__( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', 'pandastore' ) . '</p>';
			echo '<p>' . esc_html__( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', 'pandastore' ) . '</p>';
