...
( function ( $ ) {
    ...
    var LayoutBuilderModel = {
        ...
        canExtendCondition: function ( category, type = '' ) {
            ...
            return !type || ( this.schemes[ category ].scheme[ type ] && this.schemes[ category ].scheme[ type ].list );
        },
    ...
    var LayoutBuilderView = {
        ...
        init: function () {
            ...
            $( document.body )
                ...
                .on( 'change', '.alpha-scheme-list', this.changeConditionItem.bind( this ) )
        ...
        editPart: function ( e ) {
            ...
            if ( optionControls ) {
                ...
                for ( var optionName in optionControls ) {
                    ...
                    if ( 'buttonset' == control.type ) {
                        ...
                        if ( alpha_layout_vars.template_builder[ blockType ] ) {
                            if ( blockType == 'sidebar' ) {
                                control_html += '<a href="' + window.location.href.substr( 0, window.location.href.indexOf( 'wp-admin' ) + 8 ) + '/admin.php?page=alpha_sidebar" target="_blank" class="new-block-template">' + wp.i18n.__( 'Please create a new ', 'pandastore' ) + blockType + '</a>';
                            } else {
                                control_html += '<a href="' + ( window.location.href.substr( 0, window.location.href.indexOf( 'wp-admin' ) + 8 ) + '/edit.php?post_type=' + alpha_admin_vars.theme + '_template&' + alpha_admin_vars.theme + '_template_type=' + blockType ) + '" target="_blank" class="new-block-template">' + wp.i18n.__( 'Please create a new ', 'pandastore' ) + ( blockType == 'product_layout' ? wp.i18n.__( 'single product', 'pandastore' ) : blockType == 'shop_layout' ? wp.i18n.__( 'shop layout', 'pandastore' ) : blockType ) + '</a>';
                            }
                        } else {
                            control_html += '<a href="' + window.location.href.substr( 0, window.location.href.indexOf( 'wp-admin' ) + 8 ) + '/admin.php?page=alpha-setup-wizard&step=default_plugins" class="new-block-template">' + wp.i18n.__( 'Please activate ', 'pandastore' ) + alpha_admin_vars.theme_display_name + ' Core</a>';
                        }
                        control_html += '</div>';
                    ...
                    if ( optionName == 'shop_layout_template' && ( !optionValues || 'builder' != optionValues[ 'shop_layout_type' ] ) ) {
                        show = false;
                    }
                    if ( [ 'products_column', 'loadmore_type' ].includes( optionName ) && ( optionValues && 'builder' == optionValues[ 'shop_layout_type' ] ) ) {
                        show = false;
                    }
        ...
        changeOptionInput: function ( e ) {
            ...
            if ( name.startsWith( '_alpha_' ) ) {
                ...
                if ( name.indexOf( 'single_product_type' ) >= 0 ) {
                    var $template = $input.closest( '.alpha-layout-options' ).find( '.alpha-layout-control[data-option="single_product_template"]' );
                    $template.length && $template.toggle( value == 'builder' );
                } else if ( name.indexOf( 'shop_layout_type' ) >= 0 ) {
                    var $template = $input.closest( '.alpha-layout-options' ).find( '.alpha-layout-control[data-option="shop_layout_template"]' );
