...
defined( 'ABSPATH' ) || die;
?>
<h2 class="wizard-title"><?php esc_html_e( 'Install Addons', 'pandastore' ); ?></h2>
<form method="post">
	...
	if ( count( $plugins['all'] ) ) {
		?>
		<p>
			<?php echo sprintf( esc_html__( 'This will install the default addons which are used in %s', 'pandastore' ), ALPHA_DISPLAY_NAME ); //phpcs:ignore ?>
			<br>
			<?php esc_html_e( 'Please check the addons to install:', 'pandastore' ); ?>
		</p>
		...
		echo '<p class="lead">' . esc_html__( 'Good news! All addons are already installed and up to date. Please continue.', 'pandastore' ) . '</p>';
	}
	?>
