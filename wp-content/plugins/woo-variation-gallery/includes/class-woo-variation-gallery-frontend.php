...
if ( ! class_exists( 'Woo_Variation_Gallery_Frontend' ) ):
	...
	class Woo_Variation_Gallery_Frontend {
		...
		public function get_gallery_image_html( $product, $attachment_id, $options = array() ) {
			...
			$template
				= '<div class="dfdf wvg-single-gallery-image-container"><img loading="lazy" width="%d" height="%d" src="%s" class="%s" alt="%s" title="%s" data-caption="%s" data-src="%s" data-large_image="%s" data-large_image_width="%d" data-large_image_height="%d" srcset="%s" sizes="%s" %s /></div>';
