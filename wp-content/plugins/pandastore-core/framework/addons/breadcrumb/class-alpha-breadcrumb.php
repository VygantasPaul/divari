	...
	class Alpha_Breadcrumb extends Alpha_Base {
		...
		private function set_args( $atts ) {
			...
			if ( ! empty( $atts ) ) { // breadcrumb widget option
				...
				$home_icon = alpha_get_option( 'ptb_home_icon' );

				if ( ! $delimiter ) {
					$delimiter = '<i class="' . ALPHA_ICON_PREFIX . '-icon-angle-right-solid"></i>';
				}
		...
		private function get_breadcrumbs() {
			...
			// Home Link
			if ( ! is_front_page() ) {
				$this->breadcrumbs[ esc_html__( 'Home', 'pandastore-core' ) ] = apply_filters( 'woocommerce_breadcrumb_home_url', home_url() );
			}
			...
			if ( isset( $post->post_type ) && get_post_type_archive_link( $post->post_type ) && 'post' != $post->post_type ||
				( is_archive() && ! ( is_home() && ! is_front_page() && 'page' == get_option( 'show_on_front' ) ) ) ) {
			...
			if ( is_singular() ) {
				...
				if ( apply_filters( 'alpha_is_vendor_store', false ) ) {
					...
					if ( is_tag() ) {
						...
						$title                       = sprintf( esc_html__( 'Tag - %s', 'pandastore-core' ), $title );
						$this->breadcrumbs[ $title ] = '';
					} elseif ( is_tax( 'product_tag' ) ) {
						$title                       = sprintf( esc_html__( 'Product Tag - %s', 'pandastore-core' ), $title );
						$this->breadcrumbs[ $title ] = '';
					} else {
						$this->breadcrumbs[ $title ] = '';
					}
		...
		private function _get_archive_title_link() {
			...
			if ( null !== $post ) {
				...
				if ( ! empty( $_REQUEST['post_type'] ) ) {
					...
					if ( function_exists( 'wc_get_page_id' ) && ! empty( wc_get_page_id( 'shop' ) ) ) {
						$shop_page_id   = wc_get_page_id( 'shop' );
						$shop_page_name = $shop_page_id ? get_the_title( $shop_page_id ) : '';
		...
		private function _get_leaf_title( $object_type = '' ) {
			...
			switch ( $object_type ) {
				...
				case 'search':
					...
					$title = sprintf( esc_html__( 'Search - %s', 'pandastore-core' ), $search );
					break;
				case '404':
					$title = esc_html__( '404', 'pandastore-core' );
					break;
				default:
