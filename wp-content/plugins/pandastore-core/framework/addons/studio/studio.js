...
( function ( $ ) {
	...
	function setupStudioBlocks() {
		...
		function openStudio( e ) {
			...
			if ( !isRunning ) {
				...
				$( '.blocks-wrapper' ).imagesLoaded( function () {
					$( '#alpha-elementor-panel-alpha-studio, #vce-alpha-studio-trigger, #wpb-alpha-studio-trigger, #alpha-new-studio-trigger' ).removeClass( 'disabled' );
						...
						if ( !$( '.blocks-wrapper .blocks-list' ).hasClass( 'initialized' ) ) {
							...
							$( '.blocks-wrapper .blocks-section' ).on( 'scroll', function () {
								...
								if ( $wrapper.length ) {
									...
									if ( top <= 10 && !$wrapper.hasClass( 'loading' ) && parseInt( $wrapper.find( '.category-list a.active' ).data( 'total-page' ), 10 ) >= alpha_blocks_cur_page + 1 ) {
										...
										if ( parseInt( filterBy, 10 ) || 'blocks' == filterBy || '*' == filterBy || 'my' == filterBy ) {
											$wrapper.find( '.category-list a.active' ).trigger( 'click', [ alpha_blocks_cur_page + 1 ] );
		...
		function confirmPageType( e ) {
			if ( 'e' == alpha_studio.page_type || 'w' == alpha_studio.page_type ) {
				page_type = alpha_studio.page_type;
			} else {
				var new_type = '';
				if ( $( '#alpha-elementor-studio' ).is( ':checked' ) )
					new_type = 'e';
		...
		$( document.body )
			.on( 'click', '#alpha-elementor-panel-alpha-studio, #vce-alpha-studio-trigger, #wpb-alpha-studio-trigger', openStudio )
