	...
	class Alpha_Studio {
		...
		public function get_category_title( $title ) {
			$titles = apply_filters(
				...
				array(
					'demo'           => esc_html__( 'Demo', 'pandastore-core' ),
					'banner'         => esc_html__( 'Banner', 'pandastore-core' ),
					'slider'         => esc_html__( 'Slider', 'pandastore-core' ),
					'cta'            => esc_html__( 'Call To Action', 'pandastore-core' ),
					'products'       => esc_html__( 'Products', 'pandastore-core' ),
					'posts'          => esc_html__( 'Posts', 'pandastore-core' ),
					'testimonial'    => esc_html__( 'Testimonial', 'pandastore-core' ),
					'other'          => esc_html__( 'Other', 'pandastore-core' ),
					'header'         => esc_html__( 'Header', 'pandastore-core' ),
					'footer'         => esc_html__( 'Footer', 'pandastore-core' ),
					'product_layout' => esc_html__( 'Single Product', 'pandastore-core' ),
					'popup'          => esc_html__( 'Popup', 'pandastore-core' ),
					'error-404'      => esc_html__( 'Error 404', 'pandastore-core' ),
					'favourites'     => esc_html__( 'Favourites', 'pandastore-core' ),
					'my-templates'   => esc_html__( 'My Templates', 'pandastore-core' ),
				)
			);
			...
			if ( 'post.php' == $GLOBALS['pagenow'] || 'post-new.php' == $GLOBALS['pagenow'] ) {
				if ( defined( 'ELEMENTOR_VERSION' ) && function_exists( 'alpha_is_elementor_preview' ) && alpha_is_elementor_preview() ) {
					...
					add_filter(
						...
						function( $html ) {
							$html[] = array(
								'elementor' => '<li id="alpha-elementor-panel-alpha-studio"><i class="fas fa-layer-group"></i>' . sprintf( esc_html__( '%s Studio', 'pandastore-core' ), ALPHA_DISPLAY_NAME ) . '</li>',
							);
							return $html;
						},
						9
		...
		public function enqueue() {
			wp_enqueue_style( 'alpha-studio-fonts', '//fonts.googleapis.com/css?family=Jost%3A400%2C600%2C700&ver=5.2.1' );
			...
			$studio_vars = array(
				...
				'limit'   => $this->limit,
				'texts'   => array(
					'loading_failed'  => esc_html__( 'Loading failed!', 'pandastore-core' ),
					'importing_error' => esc_html__( 'There was an error when importing block. Please try again later!', 'pandastore-core' ),
					'coming_soon'     => esc_html__( 'Coming Soon...', 'pandastore-core' ),
				),
			);
		...
		public function import( $pure_return = false ) {
			...
			if ( isset( $_POST['block_id'] ) ) {
				...
				if ( is_wp_error( $block ) || ! $block || ! isset( $block['content'] ) ) {
					...
					echo json_encode( array( 'error' => esc_js( esc_html__( 'Security issue found! Please try again later.', 'pandastore-core' ) ) ) );
					die();
				}
		...
		private function get_block_categories() {
			...
			if ( ! $block_categories ) {
				...
				if ( is_wp_error( $block_categories ) || ! $block_categories ) {
					return esc_html__( 'Could not connect to the API Server! Please try again later.', 'pandastore-core' );
				}
