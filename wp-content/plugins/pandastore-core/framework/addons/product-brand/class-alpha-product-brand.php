...
class Alpha_Product_Brand {
		...
		add_filter(
			...
			function( $sections ) {
				$sections['brand'] = array(
					'title'    => esc_html__( 'Product Brand', 'pandastore-core' ),
					'priority' => 90,
					'panel'    => 'features',
				);
				return $sections;
	...
	public function add_customize_fields( $fields ) {
		...
		$fields['cs_brand_about_desc']  = array(
			'section' => 'brand',
			'type'    => 'custom',
			'label'   => esc_html__( 'Please add brand taxnomy for products. You can show and filter products by brands.', 'pandastore-core' ),
			'default' => '<p class="options-custom-description option-feature-description"><img class="description-image" src="' . ALPHA_ASSETS . '/images/admin/customizer/brand.jpg' . '" alt="Theme Option Descrpition Image"></p>',
		);
		$fields['cs_brand_title']       = array(
			'section' => 'brand',
			'type'    => 'custom',
			'label'   => '',
			'default' => '<h3 class="options-custom-title">' . esc_html__( 'Brand', 'pandastore-core' ) . '</h3>',
		);
		$fields['brand']                = array(
			'section' => 'brand',
			'type'    => 'toggle',
			'label'   => esc_html__( 'Enable Product Brand', 'pandastore-core' ),
		);
		return $fields;
	}
	...
	public function register_brand() {
		...
		$labels = array(
			'name'                       => esc_html__( 'Product brands', 'pandastore-core' ),
			'singular_name'              => esc_html__( 'Brand', 'pandastore-core' ),
			'menu_name'                  => esc_html__( 'Brands', 'pandastore-core' ),
			'search_items'               => esc_html__( 'Search brands', 'pandastore-core' ),
			'all_items'                  => esc_html__( 'All brands', 'pandastore-core' ),
			'edit_item'                  => esc_html__( 'Edit brand', 'pandastore-core' ),
			'update_item'                => esc_html__( 'Update brand', 'pandastore-core' ),
			'add_new_item'               => esc_html__( 'Add New brand', 'pandastore-core' ),
			'new_item_name'              => esc_html__( 'New brand Name', 'pandastore-core' ),
			'parent_item'                => esc_html__( 'Parent Brand', 'pandastore-core' ),
			'parent_item_colon'          => esc_html__( 'Parent Brand:', 'pandastore-core' ),
			'popular_items'              => esc_html__( 'Popular brands', 'pandastore-core' ),
			'view_item'                  => esc_html__( 'View brand', 'pandastore-core' ),
			'separate_items_with_commas' => esc_html__( 'Separate brands with commas', 'pandastore-core' ),
			'add_or_remove_items'        => esc_html__( 'Add or remove brands', 'pandastore-core' ),
			'choose_from_most_used'      => esc_html__( 'Choose from the most used brands', 'pandastore-core' ),
			'not_found'                  => esc_html__( 'No brands found', 'pandastore-core' ),
		);

		$permalinks         = get_option( 'product_brand_permalinks' );
		$product_brand_base = empty( $permalinks['product_brand_base'] ) ? _x( 'product-brand', 'slug', 'pandastore-core' ) : $permalinks['product_brand_base'];
	...
	public function permalink_fields() {
		...
		add_settings_section(
			'alpha_brand_section',
			'<span id="brand-options">' . esc_html__( 'Product brand', 'pandastore-core' ) . '</span>',
		...
		add_settings_field(
			'product_brand_slug',
			'<span class="brand-options">' . esc_html__( 'Product brand', 'pandastore-core' ) . '</span>',
		...
		add_settings_field(
			'product_brand_slug',
			'<label for="product_brand_slug">' . esc_html__( 'Product brand base', 'pandastore-core' ) . '</label>',
	...
	public function product_brand_slug_input() {
		...
		?>
		<input name="product_brand_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $brand_base ); ?>" placeholder="<?php echo esc_attr_x( 'product-brand', 'slug', 'pandastore-core' ); ?>" />
		<?php
	}
	...
	public function add_category_fields() {
		...
		<div class="form-field term-thumbnail-wrap">
			<label><?php esc_html_e( 'Thumbnail', 'pandastore-core' ); ?></label>
			...
			<script>
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame = wp.media.frames.downloadable_file = wp.media({
						title: '<?php esc_html_e( 'Choose an image', 'pandastore-core' ); ?>',
						button: {
							text: '<?php esc_html_e( 'Use image', 'pandastore-core' ); ?>'
						},
						multiple: false
					});
	...
	public function edit_category_fields( $term ) {
		...
		<tr class="form-field term-thumbail-wrap">
			<th scope="row" valign="top"><label><?php esc_html_e( 'Thumbnail', 'pandastore-core' ); ?></label></th>
			<td>
				<div id="product_brand_thumbnail" style="float: left; margin-right: 10px;">
	...
	public function add_brand_columns_tax_page( $columns ) {
		...
		$new_columns['thumb'] = esc_html__( 'Image', 'pandastore-core' );
	...
	public function print_brand_column_tax_page( $columns, $column, $id ) {
		...
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'pandastore-core' ) . '" class="wp-post-image" height="48" width="48" />';
	...
	public function add_brand_columns_products_page( $columns ) {
		...
		$show_columns['thumb'] = '<span class="wc-image tips" data-tip="' . esc_attr__( 'Image', 'pandastore-core' ) . '">' . esc_html__( 'Image', 'pandastore-core' ) . '</span>';
		$show_columns['name']  = esc_html__( 'Name', 'pandastore-core' );

		if ( wc_product_sku_enabled() ) {
			$show_columns['sku'] = esc_html__( 'SKU', 'pandastore-core' );
		}

		if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
			$show_columns['is_in_stock'] = esc_html__( 'Stock', 'pandastore-core' );
		}

		$show_columns['price']         = esc_html__( 'Price', 'pandastore-core' );
		$show_columns['product_cat']   = esc_html__( 'Categories', 'pandastore-core' );
		$show_columns['product_brand'] = esc_html__( 'Brands', 'pandastore-core' );
		$show_columns['product_tag']   = esc_html__( 'Tags', 'pandastore-core' );
		$show_columns['featured']      = '<span class="wc-featured parent-tips" data-tip="' . esc_attr__( 'Featured', 'pandastore-core' ) . '">' . esc_html__( 'Featured', 'pandastore-core' ) . '</span>';
		$show_columns['date']          = esc_html__( 'Date', 'pandastore-core' );
	...
	public function writing_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to disable custom types of content on your site', 'pandastore-core' ); ?>
		</p>
		<?php
	}
	...
	public function disable_field_html() {
		...
		<label for="<?php echo esc_attr( 'product_brand_slug' ); ?>">
			...
			<?php esc_html_e( 'Disable Brand for this site.', 'pandastore-core' ); ?>
		</label>
