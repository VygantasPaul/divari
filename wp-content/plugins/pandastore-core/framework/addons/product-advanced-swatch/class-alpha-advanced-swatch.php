	...
	class Alpha_Advanced_Swatch extends Alpha_Base {
			...
			add_filter(
				...
				function( $sections ) {
					$sections['advanced_swatch'] = array(
						'title'    => esc_html__( 'Advanced Swatch', 'pandastore-core' ),
						'panel'    => 'features',
						'priority' => 40,
					);
					return $sections;
		...
		public function add_customize_fields( $fields ) {
			...
			$fields['cs_shop_advanced_swatch_desc']        = array(
				'section' => 'advanced_swatch',
				'type'    => 'custom',
				'label'   => esc_html__( 'Instead of attributes of traditional select box type, clients could see vivid and beautiful swatches of images.', 'pandastore-core' ),
				'default' => '<p class="options-custom-description option-feature-description"><img class="description-image" src="' . esc_url( ALPHA_ASSETS . '/images/admin/customizer/image-attribute.jpg' ) . '" alt="Theme Option Descrpition Image"></p>',
			);
			$fields['cs_shop_advanced_swatch_title']       = array(
				'section' => 'advanced_swatch',
				'type'    => 'custom',
				'label'   => '',
				'default' => '<h3 class="options-custom-title">' . esc_html__( 'Advanced Swatch', 'pandastore-core' ) . '</h3>',
			);
			$fields['advanced_swatch']                     = array(
				'type'    => 'toggle',
				'label'   => esc_html__( 'Enable Advanced Swatch', 'pandastore-core' ),
				'section' => 'advanced_swatch',
			);
			$fields['new_product_period']                  = array(
				'type'    => 'number',
				'label'   => esc_html__( 'New Product Period', 'pandastore-core' ),
				'tooltip' => esc_html__( 'How many days to show new label for new products.', 'pandastore-core' ),
				'section' => 'shop_notice',
			);
			return $fields;
		}
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'alpha-advanced-swatch', alpha_core_framework_uri( '/addons/product-advanced-swatch/swatch' . ALPHA_JS_SUFFIX ), array( 'alpha-framework-async' ), ALPHA_VERSION, true );
		}
		...
		public function wc_product_listed_attributes_html( $attributes = '' ) {
			...
			foreach ( $attributes as $attribute_name => $options ) {
				// alpha_doing_quickview() || apply_filters( 'alpha_is_single_product_widget', false ) || ( alpha_is_product() && ! alpha_wc_get_loop_prop( 'name' ) ) ||
							...
							foreach ( $options as $term_id_or_slug ) {
								...
								printf(
									...
									apply_filters(
										...
										$attr_color ? ' class="color" style="background-color:' . esc_attr( $attr_color ) . '"' : '',
										$attribute_name,
