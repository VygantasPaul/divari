	...
	class Alpha_Product_Image_Comment extends Alpha_Base {
		...
		public function add_customize_fields( $fields ) {
			$fields['cs_product_reviews']         = array(
				...
				'type'     => 'custom',
				'label'    => '',
				'default'  => '<h3 class="options-custom-title">' . esc_html__( 'Product Review Images', 'pandastore-core' ) . '</h3>',
				'priority' => 30,
			);
			$fields['cs_product_reviews_alert']   = array(
				'section'  => 'product_detail',
				'type'     => 'custom',
				'label'    => '<p class="options-description"><span>Warning: </span>' . sprintf( esc_html__( 'Currently your server allows you to upload files up to %s. You can change this option in settings panel', 'pandastore-core' ), size_format( wp_max_upload_size() ) ) . '</p>',
				'priority' => 30,
			);
			$fields['product_review_image_count'] = array(
				'section'  => 'product_detail',
				'type'     => 'number',
				'label'    => esc_html__( 'Maximum Image Count per Review', 'pandastore-core' ),
				'choices'  => array(
					'min' => 0,
					'max' => 10,
				),
			...
			$fields['product_review_video_count'] = array(
				'section'  => 'product_detail',
				'type'     => 'number',
				'label'    => esc_html__( 'Maximum Video Count per Review', 'pandastore-core' ),
				'choices'  => array(
					'min' => 0,
					'max' => 10,
				),
			...
			$fields['product_review_image_size']  = array(
				'section'  => 'product_detail',
				'type'     => 'number',
				'label'    => esc_html__( 'Maximum Upload File Size (MB)', 'pandastore-core' ),
				'choices'  => array(
					'min' => 0,
			...
			$fields['cs_product_reviews_form']    = array(
				...
				'type'     => 'custom',
				'label'    => '',
				'default'  => '<h3 class="options-custom-title">' . esc_html__( 'Product Review Form', 'pandastore-core' ) . '</h3>',
				'priority' => 35,
			);
			$fields['product_review_offcanvas']   = array(
				'section'  => 'product_detail',
				'type'     => 'toggle',
				'label'    => esc_html__( 'Off Canvas Review Form', 'pandastore-core' ),
				'tooltip'  => esc_html__( 'Display review form in off-canvas.', 'pandastore-core' ),
				'priority' => 35,
			);
			return $fields;
		}
			...
			wp_localize_script(
				...
				apply_filters(
					...
					array(
						...
						'added_count_text' => esc_html__( 'Added %s images', 'pandastore-core' ),
						'error_msg'        => array(
							// translators: %s represents maximum file size.
							'size_error'      => sprintf( esc_html__( 'Maximum file size is %s', 'pandastore-core' ), $this->get_max_size( true ) ),
							// translators: %s represents image file formats.
							'mime_type_error' => sprintf( esc_html__( 'You are allowed to upload images only in %s formats.', 'pandastore-core' ), apply_filters( 'alpha_product_comment_images_mimetypes', 'jpeg, png, jpg, avi, mp4, mpeg, ogv, ts, webm, 3gp, 3g2' ) ), //phpcs:ignore
						),
					)
				)
			);
		...
		public function get_attached_image_ids_arr( $comment_id = 0 ) {
			...
			return explode( ',', get_comment_meta( $comment_id, $this->meta_key_image, true ) );
		}
		...
		public function get_attached_video_ids_arr( $comment_id = 0 ) {
			...
			return explode( ',', get_comment_meta( $comment_id, $this->meta_key_video, true ) );
		}
			...
			if ( alpha_is_product() || ( alpha_doing_ajax() && isset( $_REQUEST['action'] ) && 'alpha_reviews' == $_REQUEST['action'] ) ) {
				if ( is_user_logged_in() ) {
					...
					$image_template = '<div class="file-input form-control image-input" style="background-image: url(' . wc_placeholder_img_src( array( 80, 80 ) ) . ');">
						...
						<label class="btn-action btn-upload" title="' . esc_html__( 'Upload Media', 'pandastore-core' ) . '">
							<input type="file" accept=".jpeg, .jpg, .png" name="' . $field_name . '[]">
						</label>
						<label class="btn-action btn-remove" title="' . esc_html__( 'Remove Media', 'pandastore-core' ) . '">
						</label>
					</div>';
					...
					$upload_media .= '<p class="comment-image-notice">' . sprintf( esc_html__( 'Upload images and videos. Maximum count: %1$s, size: %2$sMB', 'pandastore-core' ), alpha_get_option( 'product_review_image_count', 2 ) + alpha_get_option( 'product_review_video_count', 2 ), alpha_get_option( 'product_review_image_size', 1 ) ) . '</p>';
					$upload_media .= '</div>';

					$fields = $upload_media . $fields;

				} else {
					$fields = '<p class="comment-image-notice">' . esc_html__( 'You have to login to add images.', 'pandastore-core' ) . '</p>' . $fields;
				}
			}
		...
		public function validate_images( $comment_meta ) {
			...
			if ( is_array( $file_names ) && count( $file_names ) > $max_count ) {
				...
				wp_die( sprintf( esc_html__( 'You can upload up to % s images to review', 'pandastore-core' ), $max_count ) );
			}

			foreach ( $file_sizes as $size ) {
				if ( $size > $max_size ) {
					// translators: maximum file size.
					wp_die( sprintf( esc_html__( 'Maximum file size is % s MB', 'pandastore-core' ), size_format( $max_size ) ) ); //phpcs:ignore
				}
			}
			...
			foreach ( $file_names as $name ) {
				if ( $name ) {
					...
					if ( ! $filetype['ext'] ) {
						...
						wp_die( sprintf( esc_html__( 'You are allowed to upload images only in %s formats .', 'pandastore-core' ), apply_filters( 'alpha_product_comment_images_mimetypes', 'jpeg, png, jpg, avi, mepg, ogv, ts, webm, 3gp, 3g2' ) ) );
					}
				}
			}
		...
		public function save_comment_images( $comment_id, $comment_approved, $comment ) {
			...
			foreach ( $files['name'] as $key => $value ) {
				if ( $files['name'][ $key ] ) {
					...
					add_post_meta( $attachment_id, '_wp_attachment_image_alt', sprintf( esc_html__( 'Attachment media of %1$s\'s review on %2$s', 'pandastore-core' ), $comment['comment_author'], $post->post_title ), true );
				}
			}
		...
		public function single_product_review_filter_select_html() {
			...
			<select name="type" class="form-control" aria-label="Review Type" data-filter="type">
				<option value="all"><?php echo esc_html__( 'All Reviews', 'pandastore-core' ); ?></option>
				<option value="images"><?php echo esc_html__( 'With Images', 'pandastore-core' ); ?></option>
				<option value="videos"><?php echo esc_html__( 'With Videos', 'pandastore-core' ); ?></option>
			</select>
			<?php
		}
	}
