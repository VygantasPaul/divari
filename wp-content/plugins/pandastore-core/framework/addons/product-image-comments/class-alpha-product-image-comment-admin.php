...
if ( ! class_exists( 'Alpha_Product_Image_Comment_Admin' ) ) {
	class Alpha_Product_Image_Comment_Admin extends Alpha_Base {
		...
		public function add_metaboxes() {
			add_meta_box( 'alpha-comment-images-metabox', ALPHA_DISPLAY_NAME . esc_html__( ' Comment Images', 'pandastore-core' ), array( $this, 'render' ), null, 'normal', 'low' );
		}
		...
		public function render() {
			...
			<div class="alpha-comment-meta-box-layout">
				...
				<div class="alpha-comment-action-wrapper">
					<button id="alpha-comment-image-upload-btn" class="button-image-upload button button-primary"><?php esc_html_e( 'Upload', 'pandastore-core' ); ?></button>
		...
		public function add_comment_list_columns( $columns ) {
			return array_merge(
				...
				array(
					'attachments' => esc_html__( 'Attachments', 'pandastore-core' ),
				),
		...
		public function add_comment_list_column( $name, $id ) {
			...
			if ( empty( $image_ids ) && empty( $video_ids ) ) {
				echo '<p>' . esc_html__( 'No Attachments', 'pandastore-core' ) . '</p>';
			} else {
					...
					foreach ( $video_ids as $video_id ) {
						if ( $video_id ) {
							...
							if ( 0 === strpos( $type, 'video' ) ) {
								$url = wp_get_attachment_url( $video_id );
								echo '<video class="" src="' . esc_url( $url ) . '" preload="metadata"></video>';
							}
