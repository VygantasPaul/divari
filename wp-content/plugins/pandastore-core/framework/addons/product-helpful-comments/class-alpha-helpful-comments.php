...
if ( ! class_exists( 'Alpha_Helpful_Comments' ) ) {
	...
	class Alpha_Helpful_Comments extends Alpha_Base {
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script( 'alpha-product-helpful-comments', alpha_core_framework_uri( '/addons/product-helpful-comments/product-helpful-comments' . ALPHA_JS_SUFFIX ), array( 'alpha-framework-async' ), ALPHA_VERSION, true );
		}
		...
		public function single_product_review_filter_select_html() {
			?>
				<select name="order" class="form-control" aria-label="Review Order" data-filter="order">
					<option value="default"><?php echo esc_html__( 'Default', 'pandastore-core' ); ?></option>
					<option value="newest"><?php echo esc_html__( 'Newest Comment', 'pandastore-core' ); ?></option>
					<option value="oldest"><?php echo esc_html__( 'Oldest Comment', 'pandastore-core' ); ?></option>
					<option value="highest"><?php echo esc_html__( 'Highest Rating', 'pandastore-core' ); ?></option>
					<option value="lowest"><?php echo esc_html__( 'Lowest Rating', 'pandastore-core' ); ?></option>
					<option value="helpful"><?php echo esc_html__( 'Helpful Comment', 'pandastore-core' ); ?></option>
					<option value="unhelpful"><?php echo esc_html__( 'Unhelpful Comment', 'pandastore-core' ); ?></option>
				</select>
			<?php
		}
			...
			if ( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'alpha-nonce' ) ) {
				...
				if ( isset( $req_posts ) ) {
					...
					if ( empty( $comment_counts->approved ) ) {
						echo esc_html__( 'No comments on this post.', 'pandastore-core' );
					}
		...
		public function display_comment_vote( $comment ) {
			...
			<div class="review-vote" id="alpha_review_vote-<?php echo absint( $comment_id ); ?>">
				<span class="comment_help btn btn-link <?php echo ( 'plus' == $status ? 'already-voted' : '' ); ?>" data-comment_id="<?php echo absint( $comment_id ); ?>" data-count="<?php echo absint( $comment_help_count ); ?>">
					<i class="far fa-thumbs-up"></i><?php esc_html_e( 'Helpful', 'pandastore-core' ); ?> (<span id="commenthelp-count-<?php echo absint( $comment_id ); ?>"><?php echo intval( $comment_help_count ); ?></span>)
				</span>
				<span class="comment_unhelp btn btn-link <?php echo ( 'minus' == $status ? 'already-voted' : '' ); ?>" data-comment_id="<?php echo absint( $comment_id ); ?>" data-count="<?php echo absint( $comment_unhelp_count ); ?>">
					<i class="far fa-thumbs-down"></i><?php esc_html_e( 'Unhelpful', 'pandastore-core' ); ?> (<span id="commentunhelp-count-<?php echo absint( $comment_id ); ?>"><?php echo intval( $comment_unhelp_count ); ?></span>)
				</span>
				<?php if ( 0 == get_current_user_id() ) : ?>
					<span class="comment_alert" style="display: none;"><?php esc_html_e( 'You have to login to vote comments.', 'pandastore-core' ); ?></span>
				<?php endif; ?>
			</div>
			<?php
		}
		...
		public function display_recommend_value( $product ) {
			...
			if ( $total_count > 0 ) {
				...
				<h4 class="recommended-value">
					...
					<?php esc_html_e( 'Recommended', 'pandastore-core' ); ?>
					<span>
						<?php
						/* translators: %1$d represents recommended count, %2$s represents total count. */
						printf( esc_html__( '(%1$d of %2$d)', 'pandastore-core' ), $recommended_count, $total_count );
						?>
					</span>
				</h4>
				<?php
