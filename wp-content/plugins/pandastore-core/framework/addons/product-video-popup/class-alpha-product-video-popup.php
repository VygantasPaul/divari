	...
	class Alpha_Product_Video_Popup extends Alpha_Base {
		...
		public function add_meta_boxes( $meta_boxes ) {
			$meta_boxes[] = array(
				...
				'title'      => esc_html__( 'Product Video', 'pandastore-core' ),
				'post_types' => array( 'product' ),
				'context'    => 'side',
				'priority'   => 'low',
				'fields'     => array(
					array(
						'name' => esc_html__( 'Video from Library', 'pandastore-core' ),
						'id'   => 'alpha_product_video_thumbnail',
						'type' => 'video',
						'std'  => false,
					),
					array(
						'name' => esc_html__( 'Video URL', 'pandastore-core' ),
						'id'   => 'alpha_product_video_popup_url',
						'type' => 'input',
						'std'  => false,
						'desc' => esc_html__( 'Enter URL of Youtube or Vimeo or specific filetypes such as mp4, webm, ogv.', 'pandastore-core' ),
					),
				),
			);
		...
		public function enqueue_scripts() {
			...
			wp_enqueue_script( 'alpha-product-video-popup', alpha_core_framework_uri( '/addons/product-video-popup/product-video-popup' . ALPHA_JS_SUFFIX ), array( 'alpha-framework-async' ), ALPHA_VERSION, true );
		}
		...
		public function get_video_viewer_btn( $buttons ) {
			return $buttons . '<button class="product-gallery-btn open-product-video-viewer ' . ALPHA_ICON_PREFIX . '-icon-movie" aria-label="' . esc_html__( 'Product Video Thumbnail', 'pandastore-core' ) . '" title="' . esc_html__( 'Product Video Thumbnail', 'pandastore-core' ) . '"></button>';
		}
	}
}
