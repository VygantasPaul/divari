	...
	class Alpha_Product_Compare extends Alpha_Base {
		...
		public function get_compare_popup_template() {
			...
			<div class="container">
				...
				<a href="<?php echo esc_url( get_permalink( wc_get_page_id( 'compare' ) ) ); ?>" class="btn btn-dark btn-rounded"><?php esc_html_e( 'Start Compare !', 'pandastore-core' ); ?></a>
			</div>
				<?php
			endif;
			...
			if ( 'mini_popup' == $this->popup_type ) :
				...
				<div class="minipopup-box">
					...
					<div class="minipopup-footer">
						...
						$product = $org_product;
						?>
						<a href="<?php echo get_permalink( wc_get_page_id( 'compare' ) ); ?>" class="btn btn-sm btn-dark btn-rounded"><?php echo esc_html__( 'Compare', 'pandastore-core' ); ?></a>
					</div>
				</div>
					<?php
				endif;
			...
			if ( $global ) {
				$fields = apply_filters(
					...
					array(
						...
						'sku'          => array(
							'name'  => esc_html__( 'Sku', 'pandastore-core' ),
							'value' => 'sku',
						),
					)
				);
			...
			switch ( $type ) {
				case 'basic':
					echo '<div class="compare-basic-info">';
						...
						echo '<a href="#" class="compare-action to-left" title="To Left" data-prduct_id="' . esc_attr( $product['id'] ) . '"><i class="' . ALPHA_ICON_PREFIX . '-icon-angle-left-solid"></i></a>';
						...
						echo '<a href="#" class="compare-action to-right" title="To Right" data-prduct_id="' . esc_attr( $product['id'] ) . '"><i class="' . ALPHA_ICON_PREFIX . '-icon-angle-right-solid"></i></a>';

						echo '<figure class="product-media"><a href="' . esc_url( get_permalink( $product['id'] ) ) . '">' . $product['basic']['image'] . '</a><div class="product-action-vertical">' . alpha_strip_script_tags( $product['basic']['add_to_cart'] ) . alpha_strip_script_tags( $product['basic']['add_to_yith'] ) . '</div></figure>';
