...
class Alpha_Skeleton extends Alpha_Base {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'alpha-skeleton', alpha_core_framework_uri( '/addons/skeleton/skeleton' . ALPHA_JS_SUFFIX ), array( 'alpha-framework-async' ), ALPHA_VERSION, true );
		wp_localize_script(
			'alpha-skeleton-js',
			'lib_skeleton',
			apply_filters(
	...
	public function sidebar_content_start() {
		...
		if ( 0 == strpos( $layout_type, 'archive_' ) || 0 == strpos( $layout_type, 'single_' ) ) {
			ob_start();
	...
	public function sidebar_classes( $class ) {
		...
		if ( ! in_array( 'top-sidebar', $class ) && ( 0 == strpos( $layout_type, 'archive_' ) || 0 == strpos( $layout_type, 'single_' ) ) ) {
			$class[] = 'skeleton-body';
		}
		return $class;
	}
	 ...
	 * @since 1.0
	 */
	public function post_loop_before_item( $classes ) {
		if ( alpha_get_loop_prop( 'widget' ) ) {
			return;
		}
		if ( ! $this->is_doing ) {
