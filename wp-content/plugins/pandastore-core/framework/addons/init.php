...
add_action(
	...
	function( $request ) {
		...
		if ( class_exists( 'WooCommerce' ) && alpha_get_feature( 'fs_plugin_woocommerce' ) ) {
			// @start feature: fs_addon_vendors
			if ( alpha_get_feature( 'fs_addon_vendors' ) && ( class_exists( 'WeDevs_Dokan' ) || class_exists( 'WCFM' ) || class_exists( 'WCMp' ) || class_exists( 'WC_Vendors' ) ) ) {
				require_once alpha_core_framework_path( ALPHA_CORE_ADDONS . '/vendors/class-alpha-vendors.php' );
			}
			// @end feature: fs_addon_vendors
			...
			if ( alpha_get_feature( 'fs_addon_product_compare' ) ) {
			...
			if ( alpha_get_feature( 'fs_addon_product_attribute_guide' ) ) {
				require_once alpha_core_framework_path( ALPHA_CORE_ADDONS . '/product-attribute-guide/product-attribute-guide.php' );
			}
			...
			if ( alpha_get_feature( 'fs_addon_product_custom_tabs' ) && $request['is_admin'] && ( $request['doing_ajax'] || $request['product_edit_page'] ) ) {
			...
			// @start feature: fs_addon_product_catalog
			if ( alpha_get_feature( 'fs_addon_product_catalog' ) ) {
				require_once alpha_core_framework_path( ALPHA_CORE_ADDONS . '/product-catalog/class-alpha-product-catalog.php' );
			}
			// @end feature: fs_addon_product_catalog
		...
		if ( alpha_get_feature( 'fs_addon_studio' ) ) {
			...
			( $request['doing_ajax'] || $request['is_preview'] || 'edit.php' == $GLOBALS['pagenow'] && isset( $_REQUEST['post_type'] ) && ALPHA_NAME . '_template' == $_REQUEST['post_type'] ) ) {
