	...
	class Alpha_Product_Data_Addons {
		...
		public function add_product_data_tab( $tabs ) {
			$tabs['alpha_data_addon'] = array(
				'label'    => ALPHA_DISPLAY_NAME . esc_html__( ' Extra Options', 'pandastore-core' ),
			...
			<div id="alpha_data_addons" class="panel woocommerce_options_panel wc-metaboxes-wrapper hidden">
					...
					<div class="wc-metabox wc-metabox-template" style="display: none;">
						<h3>
							<select class="custom_label_type" class="alpha_label_type" name="alpha_label_type" hidden>
								<option value=""><?php esc_html_e( 'Text', 'pandastore-core' ); ?></option>
								<option value="image"><?php esc_html_e( 'Image', 'pandastore-core' ); ?></option>
							</select>
							<div class="text-controls">
								<input type="text" placeholder="Label" class="label_text"  name="label_text">
								<label><?php esc_html_e( 'Color:', 'pandastore-core' ); ?></label>
								<input type="text" class="color-picker label_color" name="label_color" value="">
								<label><?php esc_html_e( 'Bg Color:', 'pandastore-core' ); ?></label>
							...
							<a href="#" class="delete"><?php esc_html_e( 'Remove', 'pandastore-core' ); ?></a>
						</h3>
					</div>
					<div class="form-field alpha_custom_labels">
						<label><?php esc_html_e( 'Custom Labels', 'pandastore-core' ); ?></label>
						<button type="button" class="button add_custom_label" id="alpha_add_custom_label"><?php esc_html_e( 'Add a Label', 'pandastore-core' ); ?></button>
						<?php echo wc_help_tip( __( 'Add custom labels for this product. Custom labels will be shown just after theme supported labels.', 'pandastore-core' ) ); ?>
						<div class="wc-metaboxes ui-sortable">
						<?php
						$alpha_custom_labels = get_post_meta( $thepostid, 'alpha_custom_labels', true );
						$alpha_custom_labels = json_decode( $alpha_custom_labels, true );
						if ( is_array( $alpha_custom_labels ) && count( $alpha_custom_labels ) ) :
							foreach ( $alpha_custom_labels as $custom_label ) :
								...
								<div class="wc-metabox wc-metabox-template">
									<h3>
										<select class="custom_label_type" class="alpha_label_type" name="alpha_label_type" hidden>
											<option value="" <?php selected( $custom_label['type'], '' ); ?>><?php esc_html_e( 'Text', 'pandastore-core' ); ?></option>
											<option value="image" <?php selected( $custom_label['type'], 'image' ); ?>><?php esc_html_e( 'Image', 'pandastore-core' ); ?></option>
										</select>
				...
				<div class="options_group">
					...
					woocommerce_wp_text_input(
						array(
							'id'          => 'alpha_extra_info',
							'label'       => esc_html__( 'Extra Info', 'pandastore-core' ),
							'desc_tip'    => true,
							'description' => esc_html__( 'Add extra info for this product. Extra info will be shown in product loop after price.', 'pandastore-core' ),
						)
					);
					?>
				</div>
				...
				<div class="toolbar">
					<button type="button" class="button save_alpha_product_options button-primary"><?php esc_html_e( 'Save options', 'pandastore-core' ); ?></button>
				</div>
			</div>
			<?php
		}
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'alpha-product-data-addons', alpha_core_framework_uri( '/addons/product-data-addons/product-data-addons-admin' . ALPHA_JS_SUFFIX ), array(), ALPHA_VERSION, true );
		...
		public function save_extra_options() {
			...
			if ( count( $alpha_custom_labels ) ) {
				update_post_meta( $post_id, 'alpha_custom_labels', json_encode( $alpha_custom_labels ) );
			} else {
