...
class Alpha_Line_Chart_Elementor_Widget extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Line Chart', 'pandastore-core' );
	}
	...
	public function _register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'description' => esc_html__( 'Input multiple labels separated by comman(,)', 'pandastore-core' ),
				)
			);
			$this->add_control(
				'data_axis_range',
			...
			$repeater->start_controls_tabs( 'bar_chart_tabs' );
					...
					$repeater->add_control(
						...
						array(
							...
							'options'   => array(
								...
								'rect'   => esc_html__( 'Square', 'pandastore-core' ),
				...
				$repeater->start_controls_tab(
					...
					$repeater->add_control(
						'point_color',
						array(
							'label' => esc_html__( 'Point Color', 'pandastore-core' ),
							'type'  => Controls_Manager::COLOR,
						)
					);
				$repeater->end_controls_tab();
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'border_type',
				array(
					'label'       => esc_html__( 'Border Type', 'pandastore-core' ),
					'type'        => Controls_Manager::SELECT,
					'description' => esc_html__( 'Set border type of chart, Smooth, Non-Smooth and Stepped', 'pandastore-core' ),
					'default'     => 'smooth',
					'options'     => array(
						'smooth'     => esc_html__( 'Smooth', 'pandastore-core' ),
						'non_smooth' => esc_html__( 'Non Smooth', 'pandastore-core' ),
						// 'stepped'    => esc_html__( 'Stepped', 'pandastore-core' ),
					),
				)
			);
			$this->add_control(
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				'chart_height',
				array(
					'label'     => esc_html__( 'Height of Chart Area', 'alpha_core' ),
			...
			$this->add_control(
				...
				array(
					...
					'description' => esc_html__( 'Set the width of border in chart', 'pandastore-core' ),
				)
			);
			$this->add_control(
				'point_size',
				array(
					'label'       => esc_html__( 'Point Size', 'alpha_core' ),
					'type'        => Controls_Manager::SLIDER,
					'description' => esc_html__( 'Set the size of point in chart. Setting 0 as value will remove point from chart', 'pandastore-core' ),
				)
			);
			$this->add_control(
				'chart_grid_color',
				array(
					'label'       => esc_html__( 'Grid Line Color', 'pandastore-core' ),
					'type'        => Controls_Manager::COLOR,
					'default'     => 'rgba(0,0,0,0.05)',
					'condition'   => array(
						'show_grid' => 'true',
					),
					'description' => esc_html__( 'Set the color of grid line', 'pandastore-core' ),
				)
			);
		$this->end_controls_section();
		...
		$this->start_controls_section(
			...
			array(
				'label'     => esc_html__( 'Label', 'alpha_core' ),
				'tab'       => Controls_Manager::TAB_STYLE,
				'condition' => array(
					'show_label' => 'true',
				),
	...
	public function get_chart_data() {
		...
		foreach ( $datum as $data ) {
			...
			$data['pointRadius']          = $settings['point_size']['size'] ? $settings['point_size']['size'] : 0;
			$data['pointHoverRadius']     = $settings['point_size']['size'] ? $settings['point_size']['size'] : 0;
