...
foreach ( $tables as $key => $table ) {
	...
	$html        .= sprintf( '<a class="' . esc_attr( $btn_class ) . '" href="' . ( empty( $table['link']['url'] ) ? '#' : esc_url( $table['link']['url'] ) ) . '" ' . ( ! empty( $table['link']['is_external'] ) ? ' target="nofollow"' : '' ) . ( ! empty( $table['link']['nofollow'] ) ? ' rel="_blank"' : '' ) . '>%1$s</a>', alpha_strip_script_tags( $button_label ) );
