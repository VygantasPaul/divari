...
extract( // @codingStandardsIgnoreLine
	shortcode_atts(
		array(
			...
			'hide_out_date'              => '',
			'show_progress'              => '',
			'count_text'                 => esc_html__( 'Sold: %s/%s', 'pandastore-core' ), //phpcs:ignore
			...
			'show_hover_shadow'          => '',
			'show_media_shadow'          => '',
			'show_info'                  => array( 'category', 'label', 'price', 'rating', 'countdown', 'addtocart', 'quickview', 'wishlist', 'compare' ),
			'addtocart_pos'              => '',
			'quickview_pos'              => '',
			'wishlist_pos'               => '',
			'page_builder'               => '',
...
if ( 'featured' == $status ) {
	...
	if ( ! empty( $viewed_products ) && count( $viewed_products ) ) {
		...
		esc_html_e( 'There\'s no item that you viewed recently.', 'pandastore-core' );
		return;
	}
...
if ( ! ( ( 'related' == $status || 'upsell' == $status ) && is_array( $product_ids ) && count( $product_ids ) == 0 ) ) {
	...
	if ( 'yes' == $filter_cat ) {
		...
		if ( count( $terms ) > 1 ) {
			...
			if ( $category_html ) {
				$category_html = '<ul class="nav-filters product-filters">' . ( 'yes' == $show_all_filter ? '<li class="nav-filter-clean"><a href="#" class="nav-filter active">' . esc_html__( 'All', 'pandastore-core' ) . '</a></li>' : '' ) . alpha_strip_script_tags( $category_html ) . '</ul>';
	...
	if ( ! $is_shop_builder ) {
		...
		if ( alpha_wc_get_loop_prop( 'total' ) ) {
			while ( have_posts() ) {
				the_post();
