...
( function ( $ ) {
    ...
    themeCoreAdmin.TemplateWizard = ( function () {
        ...
        function moveInsideAdminPanel() {
            $( '#wpbody-content .wrap:not(.alpha-admin-notices)' ).prependTo( $( '#wpbody-content .templates-builder' ) );
            $( 'div.updated, div.error, div.notice' ).not( '.inline, .below-h2' ).appendTo( $( '.alpha-admin-notices' ) );
        }
        ...
        function closeModal( $modal ) {
            ...
            // $modal
            //     .removeClass('alpha-modal-open').removeClass('alpha-modal-fade')
            //     .prev('.alpha-modal-overlay')
            //     .removeClass('alpha-modal-open').removeClass('alpha-modal-fade');
        }
        ...
        function showTemplateWizard( e ) {
            ...
            // $.magnificPopup.open({
            //     type: 'inline',
            //     mainClass: "alpha-modal mfp-fade",
            //     preloader: false,
            //     removalDelay: 350,
            //     autoFocusLast: false,
            //     showCloseBtn: true,
            //     items: {
            //         src: '#alpha_new_template'
            //     },
            //     callbacks: {
            //         open: function () {
            ...
            //             $('.alpha-modal .template-name').val('').trigger('focus');
            //             $('#alpha-new-template-id').add('#alpha-new-template-type').add('#alpha-new-template-name').val('');
            //         },
            //         close: function () {
            //             $('html').css('overflow-y', '');
        ...
        function createNewTemplate( e ) {
            ...
            $.ajax( {
                ...
                data: {
                    ...
                    page_builder: $( '#alpha-elementor-studio' ).is( ':checked' ) ? 'elementor' : 'wpbakery'
                },
                type: 'post',
                success: function ( response ) {
                    ...
                    window.location.href = $( '.alpha-add-new-template' )
                        ...
                        .replace(
                            ...
                            $( '#alpha-new-template-id' ).val() ? (
                                $( '#alpha-elementor-studio' ).is( ':checked' ) && 'post.php?post=' + post_id + '&action=elementor'
                ...
                elementor.on( 'document:loaded', function () {
                    $( 'body' )
                        ...
                        .on( 'click', '#alpha-custom-css', function ( e ) { // open custom css code panel
                            ...
                            // setTimeout(function () {
                            //     $('.elementor-control-alpha_custom_css_settings .elementor-panel-heading').trigger('click');
                            // }, 100);
                        } )
    ...
    $( document ).on( 'ready', function () {
        ...
        // ('undefined' !== typeof alpha_core_vars.builder_addons) && themeCoreAdmin.BuilderAddons();
