...
class Alpha_Price_Filter_Sidebar_Widget extends WC_Widget {

	public function __construct() {
		$this->widget_cssclass    = 'widget-price-filter alpha-price-filter';
		$this->widget_description = esc_html__( 'Display list of price to filter products.', 'pandastore-core' );
		$this->widget_id          = 'alpha-price-filter';
		$this->widget_name        = ALPHA_DISPLAY_NAME . esc_html__( ' - Price Filter', 'pandastore-core' );
		$this->id_base            = 'alpha-price-filter';
	...
	function widget( $args, $instance ) {
		...
		$steps = apply_filters( 'alpha_products_filter_price_range', $steps );

		echo '<div class="alpha-price-range-wrapper">';
		echo '<ul class="alpha-product-prices">';
		...
		if ( ! empty( $instance['show_all'] ) ) {
			...
			if ( $cur_url != $all_url ) {
				echo '<li class="' . ( $cur_url == $all_url ? 'chosen' : '' ) . '"><a href="' . alpha_woo_widget_clean_link( $all_url ) . '">' . esc_html__( 'All', 'pandastore-core' ) . '</a></li>';
			}
		}
		foreach ( $steps as $step ) {
			...
			if ( ! (int) $step[0] ) {
				...
				$format_text_escaped = str_replace( '$price', wc_price( floatval( $step[1] ) ), empty( $instance['prefix'] ) ? esc_html__( 'Under $price', 'pandastore-core' ) : $instance['prefix'] );
			} elseif ( ! isset( $step[1] ) ) {
			...
			$format_text_escaped = apply_filters( 'alpha_products_filter_price_range_html', $format_text_escaped, $step );
			$link                = $cur_url;
			...
			if ( ! ( $cur_min_price == $step[0] && $cur_max_price == $step[1] ) ) {
				if ( isset( $step[0] ) && $step[0] ) {
					$link = alpha_add_url_parameters( $link, 'min_price', $step[0] );
				} else {
					$step[0] = '';
				}
				if ( isset( $step[1] ) && $step[1] ) {
					$link = alpha_add_url_parameters( $link, 'max_price', $step[1] );
				} else {
					$step[1] = '';
				}
			} else {
		...
		echo '<form class="alpha-price-range" action="' . esc_url( $form_action ) . '">';
			echo '<input type="number" name="min_price" class="min_price" placeholder="' . $cur_symbol . esc_html__( 'min', 'pandastore-core' ) . '" value="' . ( isset( $_REQUEST['min_price'] ) ? $_REQUEST['min_price'] : '' ) . '">';
			echo '<span class="delimiter"></span>';
			echo '<input type="number" name="max_price" class="max_price" placeholder="' . $cur_symbol . esc_html__( 'max', 'pandastore-core' ) . '" value="' . ( isset( $_REQUEST['max_price'] ) ? $_REQUEST['max_price'] : '' ) . '">';
			echo wc_query_string_form_fields( null, array( 'min_price', 'max_price', 'paged' ), '', true );
			echo '<button type="submit" class="button filter_price">' . esc_html__( 'Go', 'pandastore-core' ) . '</button>';
		echo '</form>';
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'range' ) ); ?>">
				<strong><?php esc_html_e( 'Price Range', 'pandastore-core' ); ?>:</strong>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'range' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'range' ) ); ?>" value="<?php echo isset( $instance['range'] ) ? esc_attr( $instance['range'] ) : ''; ?>" placeholder="0-10, 10-50, 50-100, 100-500, 500-" />
				<small><?php esc_html_e( 'Comma separated price ranges.', 'pandastore-core' ); ?></small>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'format' ) ); ?>">
				<strong><?php esc_html_e( 'Price Range Format', 'pandastore-core' ); ?>:</strong>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'format' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'format' ) ); ?>" value="<?php echo isset( $instance['format'] ) ? esc_attr( $instance['format'] ) : ''; ?>" placeholder="$from - $to" />
				<small><?php esc_html_e( 'Input "$from" for first value and "$to" for last value.', 'pandastore-core' ); ?></small>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'prefix' ) ); ?>">
				<strong><?php esc_html_e( 'Min Price Range Format', 'pandastore-core' ); ?>:</strong>
				<?php // translators: Here "$price" is not text, is price value. ?>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'prefix' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'prefix' ) ); ?>" value="<?php echo isset( $instance['prefix'] ) ? esc_attr( $instance['prefix'] ) : ''; ?>" placeholder="<?php esc_html__( 'Under $price', 'pandastore-core' ); ?>" />
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'suffix' ) ); ?>">
				<strong><?php esc_html_e( 'Max Price Range Format', 'pandastore-core' ); ?>:</strong>
				<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'suffix' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'suffix' ) ); ?>" value="<?php echo isset( $instance['suffix'] ) ? esc_attr( $instance['suffix'] ) : ''; ?>" placeholder="$price+" />
			</label>
		</p>
		<p>
			<input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_all' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_all' ) ); ?>"<?php checked( isset( $instance['show_all'] ) && $instance['show_all'] ); ?> />
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_all' ) ); ?>"><?php esc_html_e( 'Show "All"', 'pandastore-core' ); ?></label></p>
		</p>
		<p>
			<input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'show_empty' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_empty' ) ); ?>"<?php checked( isset( $instance['show_empty'] ) && $instance['show_empty'] ); ?> />
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_empty' ) ); ?>"><?php esc_html_e( 'Show Empty Prices', 'pandastore-core' ); ?></label></p>
		</p>
		<?php
	}
