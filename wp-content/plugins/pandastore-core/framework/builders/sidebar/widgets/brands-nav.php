...
class Alpha_Brands_Nav_Sidebar_Widget extends WC_Widget {

	public function __construct() {
		$this->widget_cssclass    = 'widget widget_product_brands woocommerce widget_layered_nav woocommerce-widget-layered-nav';
		$this->widget_description = esc_html__( 'A list or dropdown of product brands.', 'pandastore-core' );
		$this->widget_id          = 'alpha_woo_product_brands';
		$this->widget_name        = ALPHA_DISPLAY_NAME . esc_html__( ' - Product Brands', 'pandastore-core' );
		$this->settings           = array(
			'title'   => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Product brands', 'pandastore-core' ),
			...
			'orderby' => array(
				...
				'options' => array(
					'order' => esc_html__( 'Brand order', 'pandastore-core' ),
	...
	function widget( $args, $instance ) {
		...
		$term_counts = alpha_filtered_term_product_counts( wp_list_pluck( $terms, 'term_id' ), 'product_brand', 'or' );
	...
	protected function layered_nav_list( $terms, $taxonomy ) {
		...
		foreach ( $terms as $term ) {
			...
			echo alpha_strip_script_tags( $term_html );
			echo '</li>';
		}
