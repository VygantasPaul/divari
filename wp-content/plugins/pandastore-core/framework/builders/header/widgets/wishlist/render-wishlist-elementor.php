...
if ( class_exists( 'YITH_WCWL' ) ) :
	...
	if ( $miniwishlist ) {
		echo '<div class="dropdown wishlist-dropdown mini-basket-dropdown ' . ( 'offcanvas' == $miniwishlist ? 'wishlist-offcanvas offcanvas-type ' : ' ' ) . $miniwishlist . '-type" data-miniwishlist-type="' . $miniwishlist . '">';
	}
	?>
	<a aria-label="Wishlist" class="wishlist <?php echo esc_attr( $type . '-type' ); ?>" href="<?php echo esc_url( $wc_link ); ?>">
		<?php if ( $show_icon ) : ?>
	...
	if ( $miniwishlist ) {
		...
		<div class="wishlist-popup dropdown-box">
			<?php
			if ( 'offcanvas' == $miniwishlist ) {
				echo '<div class="popup-header"><h3>' . esc_html__( 'Wishlist', 'pandastore-core' ) . '</h3><a class="btn btn-link btn-icon-after btn-close" href="#">' . esc_html__( 'close', 'pandastore-core' ) . '<i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-' . ( is_rtl() ? 'left' : 'right' ) . '"></i></a></div>';
			}
			?>
			<div class="widget_wishlist_content">
			<?php if ( empty( $wishlist_items ) ) : ?>
				<p class="empty-msg"><?php esc_html_e( 'No products in wishlist.', 'pandastore-core' ); ?></p>
			<?php else : ?>
				...
				<p class="compare-buttons buttons">
					<a href="<?php echo esc_url( $wc_link ); ?>" class="button btn btn-dark btn-md btn-block"><?php esc_html_e( 'Go To Wishlist', 'pandastore-core' ); ?></a>
				</p>
			<?php endif; ?>
