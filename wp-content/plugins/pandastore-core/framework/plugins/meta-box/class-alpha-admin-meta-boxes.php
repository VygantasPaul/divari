...
class Alpha_Admin_Meta_Boxes extends Alpha_Base {
	...
	public function add_meta_box( $meta_boxes ) {
		...
		$meta_tabs = array(
			'titles'  => array(
				'label' => __( 'Page Titles', 'pandastore-core' ),
				'icon'  => 'dashicons-heading',
			),
			'scripts' => array(
				'label' => __( 'Custom Scripts', 'pandastore-core' ),
		...
		$meta_fields = array(
			'page_title'    => array(
				'id'      => 'page_title',
				'name'    => __( 'Page Title', 'pandastore-core' ),
				'desc'    => '',
				'type'    => 'text',
				'std'     => '',
				'columns' => 12,
			...
			'page_subtitle' => array(
				'id'      => 'page_subtitle',
				'name'    => __( 'Page Subtitle', 'pandastore-core' ),
				'desc'    => '',
				'type'    => 'text',
				'std'     => '',
				'columns' => 12,
			...
			'page_css'      => array(
				'id'      => 'page_css',
				'name'    => __( 'Custom CSS', 'pandastore-core' ),
				'type'    => 'textarea',
				'columns' => 12,
				'rows'    => 10,
				'tab'     => 'scripts',
		...
		if ( current_user_can( 'unfiltered_html' ) ) {
			$meta_fields['page_js'] = array(
				'id'      => 'page_js',
				'name'    => __( 'Custom JS', 'pandastore-core' ),
				'type'    => 'textarea',
				'columns' => 12,
				'rows'    => 10,
				'tab'     => 'scripts',
		...
		if ( 'post' == $post_type ) {
			$meta_tabs['post']               = array(
				'label'    => __( 'Post Options', 'pandastore-core' ),
			...
			$meta_fields['supported_images'] = array(
				'id'                => 'supported_images',
				'type'              => 'file_advanced',
				'name'              => esc_html__( 'Supported Images', 'pandastore-core' ),
				'save_field'        => true,
				'label_description' => esc_html__( 'These images will be shown as slider with Featured Image.', 'pandastore-core' ),
				'tab'               => 'post',
			);
			$meta_fields['featured_video']   = array(
				'id'                => 'featured_video',
				'type'              => 'textarea',
				'name'              => esc_html__( 'Featured Video', 'pandastore-core' ),
				'save_field'        => true,
				'label_description' => esc_html__( 'Input embed code or use shortcodes. ex) iframe-tag or', 'pandastore-core' ) . ' [video src="url.mp4"]',
				'tab'               => 'post',
			);
		}
		...
		$meta_boxes[] = array(
			'title'      => sprintf( esc_html__( '%s Options', 'pandastore-core' ), ALPHA_DISPLAY_NAME ),
	...
	public function add_product_cat_fields( $tag ) {
		if ( is_object( $tag ) ) : ?>
			<tr class="form-field">
				<th scope="row"><label for="product_cat_icon"><?php esc_html_e( 'Category Icon', 'pandastore-core' ); ?></label></th>
				<td>
					<input name="product_cat_icon" id="product_cat_icon" type="text" value="<?php echo esc_html( get_term_meta( $tag->term_id, 'product_cat_icon', true ) ); ?>" placeholder="<?php esc_attr_e( 'Input icon class here...', 'pandastore-core' ); ?>">
				</td>
			</tr>
		<?php else : ?>
			<div class="form-field">
				<label for="product_cat_icon"><?php esc_html_e( 'Category Icon', 'pandastore-core' ); ?></label>
				<input name="product_cat_icon" id="product_cat_icon" type="text" placeholder="<?php esc_attr_e( 'Input icon class here...', 'pandastore-core' ); ?>">
			</div>
			<?php
		endif;
	}
	...
	public function metabox_source( $objects ) {
		$objects['meta-box'] = esc_html__( 'Meta Box', 'pandastore-core' );
		return $objects;
	}
		...
		if ( 'meta-box' == $plugin ) {
			...
			if ( 'image' == $widget ) {
				$object->add_control(
					'add_featured_image',
					array(
						'label' => esc_html__( 'Add Featured Image', 'pandastore-core' ),
			...
			$object->add_control(
				$control_key,
				array(
					'label'   => esc_html__( 'MetaBox Field', 'pandastore-core' ),
	...
	public function metabox_fields( $group = 'field' ) {
		...
		foreach ( $groups as $data ) {
			if ( false === in_array( $type, $data['post_types'] ) ) {
				continue;
			}
