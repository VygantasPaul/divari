...
class Alpha_Single_Product_Data_Tab_Elementor_Widget extends \Elementor\Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				'sp_tab_sticky',
				array(
					'label'     => esc_html__( 'Tab Sticky', 'pandastore-core' ),
					'type'      => Controls_Manager::SWITCHER,
					'condition' => array(
						'sp_tab_type' => array( 'tab' ),
					),
				)
			);
	...
	protected function render() {
		if ( apply_filters( 'alpha_single_product_builder_set_preview', false ) ) {
			...
			if ( 'tab' == apply_filters( 'alpha_single_product_data_tab_type', array( $this, 'get_tab_type' ) ) && 'theme' !== $settings['sp_tab_type'] ) {
				add_filter( 'alpha_single_product_tab_sticky_enabled', 'yes' == $settings['sp_tab_sticky'] ? '__return_true' : '__return_false' );
			}

			woocommerce_output_product_data_tabs();

			if ( 'tab' == apply_filters( 'alpha_single_product_data_tab_type', array( $this, 'get_tab_type' ) ) && 'theme' !== $settings['sp_tab_type'] ) {
				remove_filter( 'alpha_single_product_tab_sticky_enabled', 'yes' == $settings['sp_tab_sticky'] ? '__return_true' : '__return_false' );
			}
